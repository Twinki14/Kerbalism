    ...
    public static class Science
    {
        ...
        public static void Update(Vessel v, VesselData vd, ResourceInfo ec, double elapsed_s)
        {
            ...
            for (int i = xmitFiles.Count - 1; i >= 0; i--)
            {
                ...
                double transmitted = xmitFile.isInWarpCache
                    ? xmitFile.file.size
                    : Math.Min(xmitFile.file.size, remainingTransmitCapacity);
                ...
                if (xmitScienceValue > 0.0)
                {
                    ...
                    vd.scienceTransmitted +=
                        xmitFile.file.subjectData.RetrieveScience(xmitScienceValue, true, v.protoVessel, xmitFile.file);
                }
            }
        ...
        private static void GetFilesToTransmit(Vessel v, VesselData vd)
        {
            ...
            foreach (Drive drive in Drive.GetDrives(vd, true))
            {
                foreach (File f in drive.files.Values)
                {
                    ...
                    if (f.size <= 0.0 && (!warpCache.files.ContainsKey(f.subjectData) ||
                                          warpCache.files[f.subjectData].size <= 0.0))
                    {
            ...
            foreach (File f in warpCache.files.Values)
            {
                ...
                if (driveFileIndex >= 0)
                    xmitFiles.Add(new XmitFile(f, warpCache, f.subjectData.SciencePerMB, true,
                        xmitFiles[driveFileIndex].file));
                else
                    xmitFiles.Add(new XmitFile(f, warpCache, f.subjectData.SciencePerMB,
                        true)); // should not be happening, but better safe than sorry
            }
