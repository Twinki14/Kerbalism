...
namespace Kerbalism.Science
{
    ...
    public sealed class ExperimentInfo
    {
        ...
        public ExperimentInfo(ScienceExperiment stockDef, ConfigNode expInfoNode)
        {
            ...
            if (IsROC)
                Title = "ROC: " +
                        stockDef.experimentTitle; // group ROC together in the science archive (sorted by Title)
            else
            ...
            if (IsSample)
            {
                ...
                if (DataSize <= 0.0)
                {
                    Lib.Log(ExperimentId + " has DataSize=" + DataSize + ", your configuration is broken!",
            ...
            foreach (string virtualBiomeStr in expInfoNode.GetValues("VirtualBiome"))
            {
                ...
                {
                    Lib.Log("Experiment definition `{0}` has unknown VirtualBiome={1}", Lib.LogLevel.Warning,
                        ExperimentId, virtualBiomeStr);
                }
            }
            ...
            if (expInfoNode.HasValue("Situation"))
            {
                foreach (string situation in expInfoNode.GetValues("Situation"))
                {
                    ...
                    ScienceSituation scienceSituation =
                        ScienceSituationUtils.ScienceSituationDeserialize(sitAtBiome[0]);
                    ...
                    if (scienceSituation != ScienceSituation.None)
                    {
                        ...
                        if (sitAtBiome.Length == 2)
                        {
                            ...
                            else if (sitAtBiome[1].Equals("VirtualBiomes", StringComparison.OrdinalIgnoreCase) &&
                                     VirtualBiomes.Count > 0)
                            {
                    ...
                    {
                        Lib.Log("Experiment definition `{0}` has unknown situation : `{1}`", Lib.LogLevel.Warning,
                            ExperimentId, sitAtBiome[0]);
                    }
                }
            }
            else
            ...
            if (situationMask == 0)
            {
                Lib.Log("Experiment definition `{0}` : `0` situationMask is unsupported, patching to `BodyGlobal`",
                    Lib.LogLevel.Message, ExperimentId);
            ...
            uint stockBiomeMask;
            if (!ScienceSituationUtils.ValidateSituationBitMask(ref situationMask, biomeMask, out stockSituationMask,
                    out stockBiomeMask, out error))
            {
        ...
        public void ParseIncludedExperiments()
        {
            foreach (string expId in includedExperimentsId)
            {
                ...
                {
                    Lib.Log(
                        $"Experiment `{ExperimentId}` define a IncludedExperiment `{expId}`, but that experiment doesn't exist",
                        Lib.LogLevel.Warning);
                    continue;
                }
        ...
        public static void CheckIncludedExperimentsRecursion(ExperimentInfo expInfoToCheck,
            List<ExperimentInfo> chainedExperiments)
        {
            ...
            foreach (ExperimentInfo loopedExperiment in loopedExperiments)
            {
                ...
                Lib.Log(
                    $"IncludedExperiment `{loopedExperiment.ExperimentId}` in experiment `{expInfoToCheck.ExperimentId}` would result in an infinite loop in the chain and has been removed",
                    Lib.LogLevel.Warning);
            }
        ...
        public void CompileModuleInfos()
        {
            ...
            foreach (AvailablePart ap in PartLoader.LoadedPartsList)
            {
                ...
                foreach (PartModule module in ap.partPrefab.Modules)
                {
                    ...
                    if (module is ModuleScienceExperiment stockExpModule)
                    {
                        if (stockExpModule.experimentID == ExperimentId)
                        {
                            ...
                            ModuleInfo += "\n\n";
                            ModuleInfo +=
                                Lib.Color(Local.Experimentinfo_Situations, Lib.Kolor.Cyan, true); //"Situations:\n"
                ...
                if (ExperimentId == "asteroidSample" ||
                    ExperimentId.StartsWith("cometSample_", StringComparison.Ordinal))
                {
                ...
                else if (IsROC)
                {
                    ...
                    {
                        Lib.Log($"Can't parse ModuleInfo for {ExperimentId} on part={ap.name} : ROCManager is null",
                            Lib.LogLevel.Warning);
                        continue;
                    }
                    ...
                    if (rocDef != null)
                    {
                        ...
                        foreach (RocCBDefinition body in rocDef.myCelestialBodies)
                        {
                            ...
                            ModuleInfo +=
                                Lib.Color(
                                    "\n\n" + Local.Experimentinfo_smallRoc3.Format(
                                        Body.displayName.LocalizeRemoveGender()), Lib.Kolor.Cyan,
                                    true); //"Found on <<1>>'s :"
        ...
        public class BodyConditions
        {
            ...
            {
                public override bool TestCondition(CelestialBody body) =>
                    body.isHomeWorld || body.referenceBody.isHomeWorld;
            ...
            {
                public override bool TestCondition(CelestialBody body) =>
                    !Lib.IsSun(body) && Lib.IsSun(body.referenceBody);
            ...
            private class SpecificBody : BodyCondition
            {
                ...
                public SpecificBody(string bodyName)
                {
