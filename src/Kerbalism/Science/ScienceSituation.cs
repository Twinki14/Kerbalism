    ...
    public static class ScienceSituationUtils
    {
        ...
        public static bool IsBiomesRelevantForExperiment(this ScienceSituation situation, ExperimentInfo experiment)
        {
            return (experiment.BiomeMask & situation.BitValue()) != 0 ||
                   (experiment.VirtualBiomeMask & situation.BitValue()) != 0;
        }
        ...
        public static bool IsVirtualBiomesRelevantForExperiment(this ScienceSituation situation,
        ...
        public static float BodyMultiplier(this ScienceSituation situation, CelestialBody body)
        {
            ...
            switch (situation)
            {
                case ScienceSituation.Surface:
                case ScienceSituation.SrfLanded:
                    result = body.scienceValues.LandedDataValue;
                    break;
                case ScienceSituation.SrfSplashed:
                    result = body.scienceValues.SplashedDataValue;
                    break;
                case ScienceSituation.FlyingLow:
                    result = body.scienceValues.FlyingLowDataValue;
                    break;
                case ScienceSituation.Flying:
                case ScienceSituation.FlyingHigh:
                    result = body.scienceValues.FlyingHighDataValue;
                    break;
                case ScienceSituation.BodyGlobal:
                case ScienceSituation.Space:
                case ScienceSituation.InSpaceLow:
                    result = body.scienceValues.InSpaceLowDataValue;
                    break;
                case ScienceSituation.InSpaceHigh:
                    result = body.scienceValues.InSpaceHighDataValue;
                    break;
            }
        ...
        public static bool ValidateSituationBitMask(ref uint situationMask, uint biomeMask, out uint stockSituationMask,
            out uint stockBiomeMask, out string errorMessage)
        {
            ...
            {
                if (MaskHasSituation(situationMask, ScienceSituation.SrfSplashed) ||
                    MaskHasSituation(situationMask, ScienceSituation.SrfLanded))
                {
                    errorMessage +=
                        "Experiment situation definition error : `Surface` can't be combined with `SrfSplashed` or `SrfLanded`\n";
                ...
                SetSituationBitInMask(ref stockBiomeMask, ScienceSituation.SrfLanded,
                    MaskHasSituation(biomeMask, ScienceSituation.Surface));
            }

            if (MaskHasSituation(situationMask, ScienceSituation.Flying))
            {
                if (MaskHasSituation(situationMask, ScienceSituation.FlyingHigh) ||
                    MaskHasSituation(situationMask, ScienceSituation.FlyingLow))
                {
                    errorMessage +=
                        "Experiment situation definition error : `Flying` can't be combined with `FlyingHigh` or `FlyingLow`\n";
                ...
                SetSituationBitInMask(ref stockBiomeMask, ScienceSituation.FlyingHigh,
                    MaskHasSituation(biomeMask, ScienceSituation.Flying));
            }

            if (MaskHasSituation(situationMask, ScienceSituation.Space))
            {
                if (MaskHasSituation(situationMask, ScienceSituation.InSpaceHigh) ||
                    MaskHasSituation(situationMask, ScienceSituation.InSpaceLow))
                {
                    errorMessage +=
                        "Experiment situation definition error : `Space` can't be combined with `InSpaceHigh` or `InSpaceLow`\n";
                ...
                SetSituationBitInMask(ref stockBiomeMask, ScienceSituation.InSpaceLow,
                    MaskHasSituation(biomeMask, ScienceSituation.Space));
            }
            ...
            if (MaskHasSituation(situationMask, ScienceSituation.BodyGlobal))
            {
                ...
                {
                    errorMessage +=
                        "Experiment situation definition error : `BodyGlobal` can't be combined with another situation\n";
