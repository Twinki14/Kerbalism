    ...
    public static class ScienceDB
    {
        ...
        public class KeyValueList<TKey, TValue>
        {
            ...
            public TValue this[TKey key]
            {
                ...
                set
                {
                    keyValuePairs[keyValuePairs.FindIndex(p => p.Key.Equals(key))] =
                        new ObjectPair<TKey, TValue>(key, value);
                }
            }
        ...
        {
            public void AddSubject(ExperimentInfo expInfo, int bodyIndex, ScienceSituation scienceSituation,
                int biomeIndex, SubjectData subjectData)
            {
            ...
            public void RemoveSubject(ExperimentInfo expInfo, int bodyIndex, ScienceSituation scienceSituation,
                int biomeIndex, SubjectData subjectData)
            {
                BodiesSituationsBiomesSubject bodiesSituationsBiomesSubject;
                if (!TryGetValue(expInfo, out bodiesSituationsBiomesSubject))
        ...
        private static readonly Dictionary<string, SubjectData> unknownSubjectDatas =
            new Dictionary<string, SubjectData>();

        public static readonly Dictionary<string, ScienceSubject> sandboxSubjects =
            new Dictionary<string, ScienceSubject>();
        ...
        public static void Init()
        {
            ...
            foreach (string experimentId in ResearchAndDevelopment.GetExperimentIDs())
            {
                ...
                if (stockDef == null)
                {
                    Lib.Log("ScienceExperiment is null for experiment Id=" + experimentId + ", skipping...",
                    ...
                    foreach (ScienceSituation scienceSituation in ScienceSituationUtils.validSituations)
                    {
                        ...
                        if (scienceSituation.IsVirtualBiomesRelevantForExperiment(expInfo))
                        {
                            foreach (VirtualBiome virtualBiome in expInfo.VirtualBiomes)
                            {
                                ...
                                {
                                    Situation situation =
                                        new Situation(bodyIndex, scienceSituation, (int) virtualBiome);
                                ...
                                expBodiesSituationsBiomesSubject.AddSubject(expInfo, bodyIndex, scienceSituation,
                                    (int) virtualBiome, subjectData);
                            }
                        }
                        // if the biome mask says the situation is biome dependant :
                        else if (scienceSituation.IsBodyBiomesRelevantForExperiment(expInfo) && body.BiomeMap != null &&
                                 body.BiomeMap.Attributes.Length > 1)
                        {
                        ...
                        else
                        {
                            ...
                            expBodiesSituationsBiomesSubject.AddSubject(expInfo, bodyIndex, scienceSituation, -1,
                                subjectData);
                        }
                    }
                }
            ...
            Lib.Log(
                $"ScienceDB init done : {subjectCount} subjects found, total science points : {totalScience.ToString("F1")}");
        }
        ...
        public static void Load(ConfigNode node)
        {
            ...
            foreach (SubjectData subjectData in unknownSubjectDatas.Values)
            {
                ...
                expBodiesSituationsBiomesSubject.RemoveSubject(subjectData.ExpInfo, bodyIndex,
                    (ScienceSituation) scienceSituation, biomeIndex, subjectData);
            }
        ...
        public static void CreditScienceBuffers(double elapsed_s)
        {
            ...
            if (API.subjectsReceivedEventEnabled)
            {
                ...
                if (subjectsReceivedBuffer.Count > 0 &&
                    (gameTimeElapsedAPI > API.subjectsReceivedEventGameTimeInterval ||
                     realTimeElapsedAPI > API.subjectsReceivedEventRealTimeInterval))
                {
        ...
        public static SubjectData GetSubjectData(string integerSubjectId)
        {
            ...
            if (expInfo == null)
            {
                Lib.Log("Could not get the SubjectData from subject '" + integerSubjectId + "' : the experiment id '" +
                        expAndSit[0] + "' doesn't exists");
                return null;
            }
            ...
            {
                Lib.Log("Could not get the SubjectData from subject '" + integerSubjectId + "' : the situation id '" +
                        expAndSit[1] + "' isn't a valid integer");
                return null;
            }
        ...
        public static SubjectData GetSubjectDataFromStockId(string stockSubjectId, ScienceSubject RnDSubject = null,
            string extraSituationInfo = null)
        {
            SubjectData subjectData = null;
            ...
            if (expInfo == null)
            {
                Lib.Log("Could not parse the SubjectData from subjectId '" + stockSubjectId +
                        "' : the experiment id '" + expAndSit[0] + "' doesn't exists");
                return null;
            }
            ...
            string[] bodyAndBiome = expAndSit[1].Split(ScienceSituationUtils.validSituationsStrings,
            ...
            if (bodyAndBiome.Length == 1)
                ...
                situation = expAndSit[1].Substring(bodyAndBiome[0].Length,
                    expAndSit[1].Length - bodyAndBiome[0].Length - bodyAndBiome[1].Length);
            else
            {
                Lib.Log("Could not parse the SubjectData from subjectId '" + stockSubjectId +
                        "' : the situation doesn't exists");
                return null;
            }
            ...
            if (subjectBody == null)
            {
                ...
                Lib.Log("Could not parse the SubjectData from subjectId '" + stockSubjectId + "' : the body '" +
                        bodyAndBiome[0] + "' doesn't exist");
                return null;
            }
            ...
            int biomeIndex = -1;
            if (bodyAndBiome.Length == 2 &&
                ScienceSituationUtils.IsBodyBiomesRelevantForExperiment(scienceSituation, expInfo) &&
                subjectBody.BiomeMap != null)
            {
                for (int i = 0; i < subjectBody.BiomeMap.Attributes.Length; i++)
                {
                    ...
                    if (RnDSubject != null && extraSituationInfo == null && sanitizedBiome != bodyAndBiome[1])
                    {
                        ...
                        Dictionary<string, ScienceSubject> stockSubjects =
                            Lib.ReflectionValue<Dictionary<string, ScienceSubject>>(ResearchAndDevelopment.Instance,
                                "scienceSubjects");
                        ...
                        Lib.Log("RnD subject load : misformatted subject '" + stockSubjectId + "' was corrected to '" +
                                correctedSubjectId + "'");
                    }

                    if (subjectBody.BiomeMap.Attributes[i].name.Replace(" ", string.Empty)
                        .Equals(sanitizedBiome, StringComparison.OrdinalIgnoreCase))
                    {
                        biomeIndex = i;
                        break;
                    }
            ...
            if (subjectData == null)
            {
                ...
                UnknownSubjectData unknownSubjectData = new UnknownSubjectData(expInfo, vesselSituation, stockSubjectId,
                    RnDSubject, extraSituationInfo);
                subjectData = unknownSubjectData;
                unknownSubjectDatas.Add(stockSubjectId, unknownSubjectData);
                expBodiesSituationsBiomesSubject.AddSubject(subjectData.ExpInfo, bodyIndex, scienceSituation,
                    biomeIndex, subjectData);
            }
