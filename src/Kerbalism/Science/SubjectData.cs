...
namespace Kerbalism.Science
{
    public class SubjectData
    {
        ...
        public double ScienceRetrievedInKSC => ExistsInRnD
            ? (RnDSubject.scienceCap - RnDSubject.science <= 0f) ? ScienceMaxValue : RnDSubject.science
            : 0.0;
        ...
        public double PercentCollectedTotal => ScienceMaxValue == 0.0
            ? 0.0
            : (ScienceCollectedInFlight / ScienceMaxValue) + PercentRetrieved;
        ...
        public double ScienceValue(double dataSize, bool clampByScienceRetrieved = false,
            bool clampByScienceRetrievedAndCollected = false)
        {
        ...
        private int GetTimesCompleted(double percentRetrieved)
        {
            ...
            return ((int) percentRetrieved) +
                   (decimalPart < 1.0 - (Science.scienceLeftForSubjectCompleted / ScienceMaxValue) ? 0 : 1);
        }
        ...
        public void CreateSubjectInRnD()
        {
            ...
            if (RnDSubject != null)
            {
                Lib.Log("CreateSubjectInRnD : ScienceSubject " + StockSubjectId +
                        "exists already, this should not be happening !");
            }
            else
            {
                // create new subject
        ...
        public double RetrieveScience(double scienceValue, bool showMessage = false, ProtoVessel fromVessel = null,
            File file = null)
        {
            if (!ExistsInRnD)
                CreateSubjectInRnD();
            ...
            foreach (SubjectData overridenSubject in IncludedSubjects)
                scienceRetrieved += overridenSubject.RetrieveScience(scienceValue,
                    showMessage && overridenSubject.TimesCompleted == 0, fromVessel);
        ...
        private void OnSubjectCompleted(bool showMessage = false, ProtoVessel fromVessel = null, File file = null)
        {
            ...
            if (API.preventScienceCrediting)
            {
                ...
                GameEvents.OnScienceRecieved.Fire(TimesCompleted == 1 ? (float) ScienceMaxValue : 0f, RnDSubject,
                    fromVessel, false);
            ...
            Message.Post(Lib.BuildString(
                    ...
                    TimesCompleted == 1
                        ? Lib.HumanReadableScience(ScienceMaxValue, false)
                        : Lib.Color(Local.Nosciencegain, Lib.Kolor.Orange,
                            true)), //"no science gain : we already had this data"
                subjectResultText);
        }
    }
    ...
    public class UnknownSubjectData : SubjectData
    {
        ...
        public UnknownSubjectData(ExperimentInfo expInfo, Situation situation, string subjectId,
            ScienceSubject stockSubject = null, string extraSituationInfo = "") : base(expInfo, situation)
        {
            ...
            TimesCompleted = ExistsInRnD
                ? (int) (RnDSubject.science / (RnDSubject.scienceCap - Science.scienceLeftForSubjectCompleted))
                : 0;
