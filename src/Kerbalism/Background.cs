...
namespace Kerbalism
{
 ...
 function(v, p, m, module_prefab, part_prefab, availableRresources, resourceChangeRequest, elapsed_s);
            ...
            foreach (var e in Background_PMs(v))
            {
                switch (e.type)
                {
                    ...
                    case Module_type.Generator:
                        ProcessGenerator(v, e.p, e.m, e.module_prefab as ModuleGenerator, resources, elapsed_s);
                        break;
                    case Module_type.Converter:
                        ProcessConverter(v, e.p, e.m, e.module_prefab as ModuleResourceConverter, resources, elapsed_s);
                        break;
                    case Module_type.Drill:
                        ProcessDrill(v, e.p, e.m, e.module_prefab as ModuleResourceHarvester, resources, elapsed_s);
                        break; // Stock ground harvester module
                    ...
                    case Module_type.Scanner:
                        ...
                        break;
                    case Module_type.FissionGenerator:
                        ProcessFissionGenerator(v, e.p, e.m, e.module_prefab, ec, elapsed_s);
                        break;
                    case Module_type.RadioisotopeGenerator:
                        ProcessRadioisotopeGenerator(v, e.p, e.m, e.module_prefab, ec, elapsed_s);
                        break;
                    case Module_type.CryoTank:
                        ProcessCryoTank(v, e.p, e.m, e.module_prefab, resources, ec, elapsed_s);
                        break;
                    case Module_type.FNGenerator:
                        ProcessFNGenerator(v, e.p, e.m, e.module_prefab, ec, elapsed_s);
                        break;
        ...
        static void ProcessFNGenerator(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            PartModule fission_generator, ResourceInfo ec, double elapsed_s)
        {
        ...
        static void ProcessGenerator(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            ModuleGenerator generator, VesselResources resources, double elapsed_s)
        {
            // if active
        ...
        static void ProcessConverter(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            ModuleResourceConverter converter, VesselResources resources, double elapsed_s)
        {
            ...
            if (Lib.Proto.GetBool(m, "IsActivated"))
            {
                ...
                if (!full)
                {
                    ...
                    double exp_bonus = exp_level < 0
                        ...
                        : converter.EfficiencyBonus * (converter.SpecialistBonusBase +
                                                       (converter.SpecialistEfficiencyFactor * (exp_level + 1)));
        ...
        static void ProcessDrill(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            ModuleResourceHarvester harvester, VesselResources resources, double elapsed_s)
        {
            ...
            if (Lib.Proto.GetBool(m, "IsActivated"))
            {
                ...
                if (resources.GetResource(v, harvester.ResourceName).Level < harvester.FillAmount - double.Epsilon)
                {
                    ...
                    double exp_bonus = exp_level < 0
                        ...
                        : harvester.EfficiencyBonus * (harvester.SpecialistBonusBase +
                                                       (harvester.SpecialistEfficiencyFactor * (exp_level + 1)));
                    ...
                    if (abundance > harvester.HarvestThreshold)
                    {
                        ...
                        recipe.AddOutput(harvester.ResourceName,
                            abundance * harvester.Efficiency * exp_bonus * elapsed_s, true);
                        resources.AddRecipe(recipe);
                    }
                }
        ...
        static void ProcessAsteroidDrill(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            ModuleAsteroidDrill asteroid_drill, VesselResources resources, double elapsed_s)
        {
            // note: untested
                ...
                if (asteroid_info != null && asteroid_resource != null)
                {
                    ...
                    if (mass > mass_threshold && abundance > double.Epsilon)
                    {
                        ...
                        if (asteroid_drill.UseSpecialistBonus)
                        {
                            foreach (ProtoCrewMember c in Lib.CrewList(v))
                            {
                                if (c.experienceTrait.Effects.Find(k => k.Name == asteroid_drill.ExperienceEffect) !=
                                    null)
                                {
                                    exp_level = Math.Max(exp_level, c.experienceLevel);
                                }
                        ...
                        double exp_bonus = exp_level < 0
                            ...
                            : asteroid_drill.EfficiencyBonus * (asteroid_drill.SpecialistBonusBase +
                                                                (asteroid_drill.SpecialistEfficiencyFactor *
                                                                 (exp_level + 1)));
        ...
        */
        ...
        static void ProcessFissionGenerator(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            PartModule fission_generator, ResourceInfo ec, double elapsed_s)
        {
            // note: ignore heat
            ...
            double tweakable = reactor == null
                ? 1.0
                : Lib.ConfigValue(reactor.moduleValues, "CurrentPowerPercent", 100.0) * 0.01;
            ec.Produce(power * tweakable * elapsed_s, ResourceBroker.FissionReactor);
        }


        static void ProcessRadioisotopeGenerator(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            PartModule radioisotope_generator, ResourceInfo ec, double elapsed_s)
        {
        ...
        static void ProcessCryoTank(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m, PartModule cryotank,
            VesselResources resources, ResourceInfo ec, double elapsed_s)
        {
