...
namespace Kerbalism.Profile
{
    public sealed class Rule
    {
            ...
            foreach (ProtoCrewMember c in Lib.CrewList(v))
            {
                ...
                if (res != null && rate > double.Epsilon)
                {
                    ...
                    if (interval > 0.0)
                    {
                        ...
                        if (output.Length > 0)
                            ResourceCache.GetResource(v, output)
                                .UpdateIntervalRule(required * ratio, ratePerStep * ratio, broker);
                    }
                ...
                {
#if DEBUG || DEVBUILD
                    Lib.Log("Rule " + name + " kills " + c.name + " at " + rd.problem + " " + degeneration + "/" + k +
                            "/" + step + "/" + Variance(name, c, variance));
#endif
                    if (fatal_message.Length > 0)
                        Message.Post(breakdown ? Severity.breakdown : Severity.fatality,
                            Lib.ExpandMsg(fatal_message, v, c, variant));
        ...
        static double Variance(String name, ProtoCrewMember c, double variance)
        {
            ...
            double k = (double) Lib.Hash32(name + c.courage.ToString() + c.stupidity.ToString()) /
        ...
        public double
            warning_threshold; // threshold of degeneration used to show warning messages and yellow status color

        public double danger_threshold; // threshold of degeneration used to show danger messages and red status color

        public double
            fatal_threshold; // threshold of degeneration used to show fatal messages and kill/breakdown the kerbal
