...
namespace Kerbalism.Patches
{
    ...
    class ModuleDataTransmitter_GetInfo
    {
        static bool Prefix(ModuleDataTransmitter __instance, ref string __result)
        {
            ...
            var dsn1 = CommNet.CommNetScenario.RangeModel.GetMaximumRange(__instance.antennaPower,
                GameVariables.Instance.GetDSNRange(0f));
            var dsn2 = CommNet.CommNetScenario.RangeModel.GetMaximumRange(__instance.antennaPower,
                GameVariables.Instance.GetDSNRange(0.5f));
            var dsn3 = CommNet.CommNetScenario.RangeModel.GetMaximumRange(__instance.antennaPower,
                GameVariables.Instance.GetDSNRange(1f));
            ...
            specs.Add(Local.DataTransmitter_ECidle,
                Lib.Color(Lib.HumanOrSIRate(ec * Settings.TransmitterPassiveEcFactor, Lib.ECResID),
                    Lib.Kolor.Orange)); //"EC (idle)"

            if (__instance.antennaType != AntennaType.INTERNAL)
            {
                specs.Add(Local.DataTransmitter_ECTX,
                    Lib.Color(Lib.HumanOrSIRate(ec * Settings.TransmitterActiveEcFactor, Lib.ECResID),
                        Lib.Kolor.Orange)); //"EC (transmitting)"
                specs.Add("");
                specs.Add(Local.DataTransmitter_Maxspeed,
                    Lib.HumanReadableDataRate(__instance.DataRate)); //"Max. speed"
            }
