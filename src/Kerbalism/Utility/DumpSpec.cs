...
namespace Kerbalism.Utility
{
    ...
    public sealed class DumpSpecs
    {
        ...
        public DumpSpecs(string always_dump, string dump_valves)
        {
            ...
            if (always_dump.Length == 0 || string.Equals(always_dump, "false", StringComparison.OrdinalIgnoreCase))
            {
                // if dump_valves is empty or false then don't do anything
                if (dump_valves.Length > 0 && !string.Equals(dump_valves, "false", StringComparison.OrdinalIgnoreCase))
                {
            ...
            else if (string.Equals(always_dump, "true", StringComparison.OrdinalIgnoreCase))
            {
        ...
        public sealed class ActiveValve
        {
            ...
            public string ValveTitle => current < _dumpSpecs.dumpValvesTitles.Count
                ? _dumpSpecs.dumpValvesTitles[current]
                : string.Empty;
