...
namespace Kerbalism.Database
{
    ...
    {
        public Dictionary<uint, List<SunShieldingPartData>> habitatShieldings =
            new Dictionary<uint, List<SunShieldingPartData>>();

        private int habitatIndex = -1;
        private List<Habitat> habitats;

        public double MaxPressure => maxPressure;
        ...
        public double AverageHabitatRadiation(double radiation)
        {
            ...
            foreach (var shieldingPartsList in habitatShieldings.Values)
            {
                ...
                foreach (var shieldingInfo in shieldingPartsList)
                {
                    ...
                    result += Radiation.DistanceRadiation(bremsstrahlung, Math.Max(1, shieldingInfo.distance)) /
                              10; //Gamma radiation has 1/10 the quality factor of SEP
                }
