...
namespace Kerbalism.Database
{
    public class VesselData
    {
        ...
        private List<ResourceUpdateDelegate>
            resourceUpdateDelegates = null; // all part modules that have a ResourceUpdate method
            ...
            if (!parts.TryGetValue(flightID, out pd))
            {
                foreach (Part p in Vessel.parts)
                {
                    if (p.flightID == flightID)
                    {
                        ...
                        Lib.LogDebug("VesselData : newly created part '{0}' added to vessel '{1}'",
                            Lib.LogLevel.Message, p.partInfo.title, Vessel.vesselName);
                    }
                }
            }
        ...
        bool isSerenityGroundController;
        ...
        /// </summary>
        public double EnvSolarFluxTotal => solarFluxTotal;
        ...
        public double EnvSunlightFactor => sunlightFactor;
        ...
        List<SunInfo> sunsInfo;

        public VesselSituations VesselSituations => vesselSituations;
        VesselSituations vesselSituations;

        public class SunInfo
        {
            /// <summary> reference to the sun/star</summary>
            public Sim.SunData SunData => sunData;
            ...
            public double SunlightFactor => sunlightFactor;
            ...
            /// </summary>
            public double SolarFlux => solarFlux;
            ...
            public static void UpdateSunsInfo(VesselData vd, Vector3d vesselPosition, double elapsedSeconds)
            {
                ...
                foreach (Sim.SunData sunData in Sim.suns)
                {
                    ...
                    if (vd.isAnalytic)
                    {
                        ...
                        Lib.DirectionAndDistance(vesselPosition, sunInfo.sunData.body, out sunInfo.direction,
                            out sunInfo.distance);
                    ...
                    else
                    {
                        ...
                        sunInfo.sunlightFactor = Sim.IsBodyVisible(v, vesselPosition, sunData.body, vd.visibleBodies,
                            out sunInfo.direction, out sunInfo.distance)
                            ? 1.0
                            : 0.0;
                        // get atmospheric absorbtion
        ...
        public int CrewCount => crewCount;
        ...
        public double DrivesFreeSpace => drivesFreeSpace;

        double drivesFreeSpace = 0.0;

        /// <summary>data capacity of all public drives</summary>
        public double DrivesCapacity => drivesCapacity;

        double drivesCapacity = 0.0;
        ...
        public double SolarPanelsAverageExposure => solarPanelsAverageExposure;

        double solarPanelsAverageExposure = -1.0;

        private List<double>
            solarPanelsExposure =
                new List<double>(); // values are added by SolarPanelFixer, then cleared by VesselData once solarPanelsAverageExposure has been computed
        ...
        public void Update(Vessel v)
        {
            ...
            else
            {
                ...
                if (!IsSimulated)
                {
                    Lib.LogDebug("VesselData : id '" + VesselId + "' (" + Vessel.vesselName +
                                 ") is now simulated (wasn't previously)");
                    IsSimulated = true;
            ...
            List<ResourceInfo>
                allResources =
                    resources.GetAllResources(
                        Vessel); // there might be some performance to be gained by caching the list of all resource
        ...
        {
            Lib.LogDebug("Decoupling vessel '{0}' from vessel '{1}'", Lib.LogLevel.Message, newVessel.vesselName,
                oldVessel.vesselName);
            ...
            Lib.LogDebug("Decoupling complete for new vessel, vd.partcount={1}, v.partcount={2} ({0})",
                Lib.LogLevel.Message, newVessel.vesselName, newVD.parts.Count, newVessel.parts.Count);
            Lib.LogDebug("Decoupling complete for old vessel, vd.partcount={1}, v.partcount={2} ({0})",
                Lib.LogLevel.Message, oldVessel.vesselName, oldVD.parts.Count, oldVessel.parts.Count);
        }
        ...
        {
            Lib.LogDebug("Coupling part '{0}' from vessel '{1}' to vessel '{2}'", Lib.LogLevel.Message,
                data.from.partInfo.title, data.from.vessel.vesselName, data.to.vessel.vesselName);
            ...
            if (fromVD == toVD)
            {
                if (!toVD.parts.ContainsKey(data.from.flightID))
                {
                    ...
                    Lib.LogDebug("VesselData : newly created part '{0}' added to vessel '{1}'", Lib.LogLevel.Message,
                        data.from.partInfo.title, data.to.vessel.vesselName);
                }
            ...
            Lib.LogDebug("Coupling complete to   vessel, vd.partcount={1}, v.partcount={2} ({0})", Lib.LogLevel.Message,
                toVessel.vesselName, toVD.parts.Count, toVessel.parts.Count);
            Lib.LogDebug("Coupling complete from vessel, vd.partcount={1}, v.partcount={2} ({0})", Lib.LogLevel.Message,
                fromVessel.vesselName, fromVD.parts.Count, fromVessel.parts.Count);
        }
        ...
        internal static void OnPartWillDie(Part part)
        {
            ...
            Lib.LogDebug("Removing dead part, vd.partcount={0}, v.partcount={1} (part '{2}' in vessel '{3}')",
                Lib.LogLevel.Message, vd.parts.Count, part.vessel.parts.Count, part.partInfo.title,
                part.vessel.vesselName);
        }
        ...
        public VesselData(Vessel vessel)
        {
            ...
            Lib.LogDebug("VesselData ctor (new vessel) : id '" + VesselId + "' (" + Vessel.vesselName +
        ...
        public VesselData(ProtoVessel protoVessel, ConfigNode node)
        {
            ...
            if (node == null)
            {
                ...
                Lib.LogDebug("VesselData ctor (created from protovessel) : id '" + VesselId + "' (" +
                             protoVessel.vesselName + "), part count : " + parts.Count);
            }
            else
            {
                Load(node);
                Lib.LogDebug("VesselData ctor (loaded from database) : id '" + VesselId + "' (" +
                             protoVessel.vesselName + "), part count : " + parts.Count);
            }
