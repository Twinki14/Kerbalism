...
namespace Kerbalism.Comms
{
    public class CommHandlerCommNetBase : CommHandler
    {
        ...
        protected override bool NetworkIsReady =>
            CommNetNetwork.Initialized && CommNetNetwork.Instance?.CommNet != null;
        ...
        protected override void UpdateNetwork(ConnectionInfo connection)
        {
            ...
            {
                double antennaPower = link.end.isHome
        ...
        {
            MethodInfo CommNetVessel_OnNetworkPreUpdate_Info =
            ...
            Loader.HarmonyInstance.Patch(CommNetVessel_OnNetworkPreUpdate_Info,
                null,
                new HarmonyMethod(AccessTools.Method(typeof(CommHandlerCommNetBase),
                    nameof(CommNetVessel_OnNetworkPreUpdate_Postfix))));

            MethodInfo CommNetVessel_OnNetworkPostUpdate_Info =
            ...
            MethodInfo CommNetVessel_GetSignalStrengthModifier_Info = AccessTools.Method(typeof(CommNetVessel),
                nameof(CommNetVessel.GetSignalStrengthModifier));

            Loader.HarmonyInstance.Patch(CommNetVessel_GetSignalStrengthModifier_Info,
                new HarmonyMethod(AccessTools.Method(typeof(CommHandlerCommNetBase),
                    nameof(CommNetVessel_GetSignalStrengthModifier_Prefix))));
        }
        ...
        private static void CommNetVessel_OnNetworkPreUpdate_Postfix(CommNetVessel __instance, ref bool ___inPlasma,
            ref double ___plasmaMult)
        {
            ...
            if (vd.EnvStormRadiation > 0.0)
            {
                ___inPlasma = true;
                ___plasmaMult =
                    vd.EnvStormRadiation * 2.0 /
                    PreferencesRadiation.Instance
                        .StormRadiation; // We should probably have a threshold setting instead of this hardcoded formula
                ___plasmaMult = Math.Max(1.0 - ___plasmaMult, 0.0);
            }
        }

        // apply storm radiation factor to the comm strength multiplier used by stock for plasma blackout
        private static bool CommNetVessel_GetSignalStrengthModifier_Prefix(CommNetVessel __instance, bool ___canComm,
            bool ___inPlasma, double ___plasmaMult, out double __result)
        {
            if (!___canComm)
            {
                __result = 0.0;
