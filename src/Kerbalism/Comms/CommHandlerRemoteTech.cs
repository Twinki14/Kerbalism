...
namespace Kerbalism.Comms
{
    public class CommHandlerRemoteTech : CommHandler
    {
        ...
        protected override void UpdateNetwork(ConnectionInfo connection)
        {
            ...
            connection.target_name = connection.Status == LinkStatus.direct_link
                ? Lib.Ellipsis("DSN: " + (RemoteTech.NameTargetsKSC(vd.VesselId) ?? ""), 20)
            ...
            else
            {
                ...
                foreach (Guid id in controlPath)
                {
                    ...
                    // satellite name
                    controlPoint[0] =
                        Lib.Ellipsis(RemoteTech.GetSatelliteName(i) + " \\ " + RemoteTech.GetSatelliteName(id), 50);
                    // signal strength
        ...
        protected override void UpdateTransmitters(ConnectionInfo connection, bool searchTransmitters)
        {
            ...
            if (v.loaded)
            {
                ...
                foreach (PartModule pm in loadedTransmitters)
                {
                    ...
                    else
                    {
                        ...
                        {
                            double dataRate = (Lib.ReflectionValue<float>(pm, "RTPacketSize") /
                                               Lib.ReflectionValue<float>(pm, "RTPacketInterval"));
                ...
                foreach (UnloadedTransmitter mdt in unloadedTransmitters)
                {
                    ...
                    else
                    {
                        ...
                        if (packet_size != null && packet_Interval != null)
                        {
                            ...
                            if (dataRate > highestDataRate)
                            {
                                float? packetResourceCost =
                                    Lib.SafeReflectionValue<float>(mdt.prefab, "RTPacketResourceCost");
            ...
            connection.ec_idle *=
                Settings
                    .TransmitterPassiveEcFactorRT; // apply passive factor to "internal" antennas always-consumed rate
        ...
        public override double GetTransmissionCost(double transmittedTotal, double elapsed_s)
        {
            ...
            return ((vd.Connection.ec - vd.Connection.ec_idle) * (1 - strength)) *
                   (transmittedTotal / (vd.Connection.rate * elapsed_s));
        }
                ...
                for (int i = 0; i < part_prefab.Modules.Count; i++)
                {
                    if (part_prefab.Modules[i].moduleName == "ModuleRTAntenna")
                    {
                        ...
                        else
                        {
                            ...
                            Lib.LogDebug(
                                $"Could not find a ModuleRTAntenna at index {i} on part {pps.partName} on vessel {v.protoVessel.vesselName}",
                    ...
                    else if (part_prefab.Modules[i].moduleName == "ModuleRTAntennaPassive")
                    {
                        ...
                        else
                        {
                            ...
                            Lib.LogDebug(
                                $"Could not find a ModuleRTAntennaPassive at index {i} on part {pps.partName} on vessel {v.protoVessel.vesselName}",
                                Lib.LogLevel.Warning);
                        }
