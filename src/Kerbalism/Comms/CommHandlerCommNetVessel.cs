        ...
        protected override void UpdateTransmitters(ConnectionInfo connection, bool searchTransmitters)
        {
            ...
            else
            {
                ...
                foreach (UnloadedTransmitter mdt in unloadedTransmitters)
                {
                    ...
                    if (!Lib.Proto.GetBool(mdt.protoTransmitter, "isEnabled", true) ||
                        !Lib.Proto.GetBool(mdt.protoTransmitter, "canComm", true))
                        continue;
            ...
            connection.ec_idle *=
                Settings.TransmitterPassiveEcFactor; // apply passive factor to "internal" antennas always-consumed rate
            connection.ec_idle +=
                connection.ec * Settings.TransmitterPassiveEcFactor; // add "transmit" antennas always-consumed rate
            connection.ec *=
                Settings.TransmitterActiveEcFactor; // adjust "transmit" antennas transmit-only rate by the factor
                ...
                for (int i = 0; i < part_prefab.Modules.Count; i++)
                {
                    if (part_prefab.Modules[i] is ModuleDataTransmitter mdt)
                    {
                        ...
                        else
                        {
                            ...
                            Lib.LogDebug(
                                $"Could not find a ModuleDataTransmitter or derivative at index {i} on part {pps.partName} on vessel {v.protoVessel.vesselName}",
