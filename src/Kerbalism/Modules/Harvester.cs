...
namespace Kerbalism.Modules
{
    public class Harvester : PartModule, IAnimatedModule, IModuleInfo, ISpecifics, IContractObjectiveModule
    {
        ...
        [KSPField]
        public float
            length = 5f; // tolerable distance between drill head and the ground (length of the extendible part)
        ...
        public string issue = string.Empty; // if not empty, the reason why resource can't be harvested

        // show abundance level
        [KSPField(guiActive = false, guiName = "_")]
        ...
        [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = true,
            guiName = "#KERBALISM_Harvester_simulatedabundance")]
        ...
        public void Update()
        {
            ...
            if (Lib.IsEditor())
            {
                Events["Toggle"].guiName =
                    Lib.StatusToggle(title,
                        running ? Local.Harvester_running : Local.Harvester_stopped); //"running""stopped"
            }
            ...
            if (Lib.IsFlight() && ResourceMap.Instance != null)
            {
                ...
                if (part.IsPAWVisible())
                {
                    ...
                    if (deployed)
                    {
                        ...
                        Abundance = abundance > double.Epsilon
                            ? Lib.HumanReadablePerc(abundance, "F2")
                            : Local.Harvester_none; //"none"
                    }
                }
            }
        }

        public static double AdjustedRate(Harvester harvester, CrewSpecs engineer_cs, List<ProtoCrewMember> crew,
            double abundance)
        {
        ...
        public void FixedUpdate()
        {
            ...
            {
                ResourceUpdate(vessel, this, min_abundance, Kerbalism.System.Kerbalism.elapsed_s);
            }
        }
        ...
        public static void BackgroundUpdate(Vessel v, ProtoPartModuleSnapshot m, Harvester harvester, double elapsed_s)
        {
            if (Lib.Proto.GetBool(m, "deployed") && Lib.Proto.GetBool(m, "running") &&
                Lib.Proto.GetString(m, "issue").Length == 0)
            {
        ...
        // action groups
        [KSPAction("#KERBALISM_Harvester_Action")]
        ...
        public override string GetInfo()
        {
            ...
            {
                case 0:
                    source = Local.Harvester_source1;
                    break; //"the surface"
                case 1:
                    source = Local.Harvester_source2;
                    break; //"the ocean"
                case 2:
                    source = Local.Harvester_source3;
                    break; //"the atmosphere"
                case 3:
                    source = Local.Harvester_source4;
                    break; //"space"
            }

            string desc =
                Local.Harvester_generatedescription.Format(resource,
                    source); //Lib.BuildString("Extract ", , " from ", )
        ...
        public Specifics Specs()
        {
            ...
            if (ec_rate > double.Epsilon)
                specs.Add(Local.Harvester_info7, Lib.HumanOrSIRate(ec_rate, Lib.ECResID)); //"ec consumption"
            return specs;
        }
