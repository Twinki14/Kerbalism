...
using Animator = Kerbalism.Utility.Animator;
...
namespace Kerbalism.Modules
{
    public class Experiment : PartModule, ISpecifics, IModuleInfo, IPartMassModifier, IConfigurable, IMultipleDragCube
    {
        ...
        [KSPField]
        public bool sample_collecting = false; // if set to true, the experiment will generate mass out of nothing
        ...
        [KSPField]
        public string crew_operate = string.Empty; // operator crew. if set, crew has to be on vessel while recording

        [KSPField]
        public string
            crew_reset = string.Empty; // reset crew. if set, experiment will stop recording after situation change

        [KSPField]
        public string
            crew_prepare =
                string.Empty; // prepare crew. if set, experiment will require crew to set up before it can start recording
        ...
        public bool
            use_animation_group =
                false; // if true, deploy/retract animations will managed by the first found ModuleAnimationGroup
        ...
        public static bool IsRunning(ExpStatus status)
            => status == ExpStatus.Running || status == ExpStatus.Forced || status == ExpStatus.Waiting ||
               status == ExpStatus.Issue;
        ...
        public override void OnStart(StartState state)
        {
            ...
            if (ExpInfo == null)
            {
                ...
                Lib.Log($"ExpInfo for experiment_id `{experiment_id}` is null, does the config definition exists ?",
                    Lib.LogLevel.Error);
                return;
            }
        ...
        private void FirstStart()
        {
            ...
            if (!sample_collecting && ExpInfo.SampleMass > 0.0 && remainingSampleMass == 0)
            {
                ...
                if (double.IsNaN(remainingSampleMass))
                    Lib.LogDebug(
                        "remainingSampleMass is NaN on first start " + ExpInfo.ExperimentId + " " + ExpInfo.SampleMass +
                        " / " + sample_amount, Lib.LogLevel.Error);
            }
        }
        ...
        public virtual void Update()
        {
            ...
            if (Lib.IsFlight())
            {
                ...
                if (prepare_cs == null || didPrepare || (hide_when_unavailable && status != ExpStatus.Issue))
                {
                    ...
                    if (subject != null)
                    {
                        Events["ToggleEvent"].guiName = Lib.StatusToggle(
                            Lib.Ellipsis(ExpInfo.Title, Styles.ScaleStringLength(25)), StatusInfo(status, issue));
                        Events["ShowPopup"].guiName = Lib.StatusToggle(Local.StatuToggle_info,
                            Lib.BuildString(ScienceValue(Subject), " ",
                                State == RunningState.Forced
                                    ? subject.PercentCollectedTotal.ToString("P0")
                                    : RunningCountdown(ExpInfo, Subject, data_rate, prodFactor))); //"info"
                    }
                    else
                    {
                        Events["ToggleEvent"].guiName = Lib.StatusToggle(
                            Lib.Ellipsis(ExpInfo.Title, Styles.ScaleStringLength(25)), StatusInfo(status, issue));
                        Events["ShowPopup"].guiName = Lib.StatusToggle(Local.StatuToggle_info,
                            vd.VesselSituations.FirstSituationTitle); //"info"
                    }
                }
                else
                {
                ...
                Events["Prepare"].guiName =
                    Lib.BuildString(Local.Module_Experiment_Prepare + " <b>", ExpInfo.Title, "</b>"); //Prepare
                Events["Prepare"].active = !didPrepare && prepare_cs != null && subject == null;

                Events["Reset"].guiName =
                    Lib.BuildString(Local.Module_Experiment_Reset + " <b>", ExpInfo.Title, "</b>"); //Reset
        ...
        public virtual void FixedUpdate()
        {
            ...
            prodFactor = RunningUpdate(
                ...
                ExpInfo,
                expState,
                Kerbalism.System.Kerbalism.elapsed_s,
                ref situationId,
        ...
        public virtual void BackgroundUpdate(Vessel v, VesselData vd, ProtoPartModuleSnapshot m, ResourceInfo ec,
            ...
            ref int lastSituationId, ref double remainingSampleMass, out SubjectData subjectData, out string mainIssue)
        {
            ...
            if (!expInfo.IsSample)
            {
                ...
                if (double.IsNaN(available))
                    Lib.LogStack("drive.FileCapacityAvailable() returned NaN", Lib.LogLevel.Error);
                ...
                if (drive.GetFileSend(subjectData.Id))
                {
                    ...
                    if (double.IsNaN(available))
                        Lib.LogStack("warpDrive.FileCapacityAvailable() returned NaN", Lib.LogLevel.Error);
                }
            }
            else
            {
            ...
            if (double.IsNaN(chunkSize))
                Lib.Log(
                    "chunkSize is NaN " + expInfo.ExperimentId + " " + chunkSizeMax + " / " + prodFactor + " / " +
                    available + " / " + ec.Amount + " / " + prefab.ec_rate + " / " + prefab.data_rate,
                    Lib.LogLevel.Error);

            if (double.IsNaN(massDelta))
                Lib.Log(
                    "mass delta is NaN " + expInfo.ExperimentId + " " + expInfo.SampleMass + " / " + chunkSize + " / " +
                    expInfo.DataSize, Lib.LogLevel.Error);
#endif
            ...
            if (!expInfo.IsSample)
            {
                if (warpDrive != null)
                {
                    ...
                    if (chunkSize > s) // only write to persisted drive if the data cannot be transmitted in this tick
                        ...
                        (!drive.files
                            .ContainsKey(
                                subjectData)) // if everything is transmitted, create an empty file so the player know what is happening
        ...
        private static Drive GetDrive(VesselData vesselData, uint hdId, double chunkSize, SubjectData subjectData)
        {
            ...
            if (hdId != 0)
                ...
                drive = isFile
                    ? Drive.FileDrive(vesselData, chunkSize)
                    : Drive.SampleDrive(vesselData, chunkSize, subjectData);
            UnityEngine.Profiling.Profiler.EndSample();
            return drive;
        }

        private static bool HasRequiredResources(Vessel v, List<ObjectPair<string, double>> defs, VesselResources res,
            out string issue)
        {
            issue = string.Empty;
            if (defs.Count < 1)
        ...
        public static RunningState ProtoToggle(Vessel v, Experiment prefab, ProtoPartModuleSnapshot protoModule,
            bool setForcedRun = false)
        {
        ...
        private static void ProtoSetState(Vessel v, Experiment prefab, ProtoPartModuleSnapshot protoModule,
            RunningState expState)
        {
            ...
            var newStatus = GetStatus(expState,
                ScienceDB.GetSubjectData(ScienceDB.GetExperimentInfo(prefab.experiment_id),
                    prefab.GetSituation(v.KerbalismData())),
        ...
        [KSPEvent(guiActiveUnfocused = true, guiActive = true, guiActiveEditor = true, guiName = "_", active = true,
            groupName = "Science", groupDisplayName = "#KERBALISM_Group_Science")] //Science
        public void ToggleEvent()
        {
            Toggle();
        }

        [KSPEvent(guiActiveUnfocused = true, guiActive = true, guiName = "_", active = true, groupName = "Science",
            groupDisplayName = "#KERBALISM_Group_Science")] //Science
        public void ShowPopup()
        {
            new ExperimentPopup(vessel, this, part.flightID, part.partInfo.title);
        }

        [KSPEvent(guiActiveUnfocused = true, guiName = "_", active = false, groupName = "Science",
            groupDisplayName = "#KERBALISM_Group_Science")] //Science
        public void Prepare()
        {
        ...
        public static string RunningStateInfo(RunningState state)
        {
            ...
            {
                case RunningState.Stopped:
                    return Lib.Color(Local.Module_Experiment_runningstate1, Lib.Kolor.Yellow); //"stopped"
                case RunningState.Running:
                    return Lib.Color(Local.Module_Experiment_runningstate2, Lib.Kolor.Green); //"started"
                case RunningState.Forced:
                    return Lib.Color(Local.Module_Experiment_runningstate3, Lib.Kolor.Red); //"forced run"
                case RunningState.Broken:
                    return Lib.Color(Local.Module_Experiment_runningstate4, Lib.Kolor.Red); //"broken"
        ...
        public static string StatusInfo(ExpStatus status, string issue = null)
        {
            switch (status)
            {
                case ExpStatus.Stopped:
                    return Lib.Color(Local.Module_Experiment_runningstate1, Lib.Kolor.Yellow); //"stopped"
                case ExpStatus.Running:
                    return Lib.Color(Local.Module_Experiment_runningstate5, Lib.Kolor.Green); //"running"
                case ExpStatus.Forced:
                    return Lib.Color(Local.Module_Experiment_runningstate3, Lib.Kolor.Red); //"forced run"
                case ExpStatus.Waiting:
                    return Lib.Color(Local.Module_Experiment_runningstate6, Lib.Kolor.Science); //"waiting"
                case ExpStatus.Broken:
                    return Lib.Color(Local.Module_Experiment_runningstate4, Lib.Kolor.Red); //"broken"
                case ExpStatus.Issue:
                    return Lib.Color(string.IsNullOrEmpty(issue) ? Local.Module_Experiment_issue_title : issue,
                        Lib.Kolor.Orange); //"issue"
                default: return string.Empty;
            }
        }

        public static string RunningCountdown(ExperimentInfo expInfo, SubjectData subjectData, double dataRate,
            double prodFactor = 1.0, bool compact = true)
        {
            double count;
            if (subjectData != null)
        ...
        {
            if (subjectData != null)
                return Lib.BuildString(Lib.HumanReadableScience(subjectData.ScienceCollectedTotal), " / ",
                    Lib.HumanReadableScience(subjectData.ScienceMaxValue));
            else
        ...
        public Specifics Specs()
        {
            ...
            if (Requirements.Requires.Length > 0)
            {
                ...
                foreach (ExperimentRequirements.RequireDef req in Requirements.Requires)
                    specs.Add(Lib.BuildString("• <b>", ReqName(req.require), "</b>"),
                        ReqValueFormat(req.require, req.value));
            }
        ...
        public static Specifics SpecsWithoutRequires(ExperimentInfo expInfo, Experiment prefab)
        {
            ...
            if (expInfo.SampleMass == 0.0)
            {
                ...
                specs.Add(Local.Module_Experiment_Specifics_info2,
                    Lib.HumanReadableDataRate(prefab.data_rate)); //"Data rate"
                specs.Add(Local.Module_Experiment_Specifics_info3,
                    Lib.HumanReadableDuration(expSize / prefab.data_rate)); //"Duration"
            }
            else
            {
                specs.Add(Local.Module_Experiment_Specifics_info4,
                    Lib.HumanReadableSampleSize(expSize)); //"Sample size"
                specs.Add(Local.Module_Experiment_Specifics_info5,
                    Lib.HumanReadableMass(expInfo.SampleMass)); //"Sample mass"
                ...
                specs.Add(Local.Module_Experiment_Specifics_info7_sample,
                    Lib.HumanReadableDuration(expSize / prefab.data_rate)); //"Duration"
            }
            ...
            {
                specs.Add(string.Empty);
                specs.Add(Lib.Color(Local.Module_Experiment_Specifics_Situations, Lib.Kolor.Cyan,
                    true)); //"Situations:"
            ...
            if (prefab.crew_prepare.Length > 0)
            {
                ...
                specs.Add(Local.Module_Experiment_Specifics_info10,
                    cs ? cs.Info() : Local.Module_Experiment_Specifics_info10_none); //"Preparation""none"
            }

            if (prefab.crew_operate.Length > 0)
            {
                var cs = new CrewSpecs(prefab.crew_operate);
                specs.Add(Local.Module_Experiment_Specifics_info11,
                    cs ? cs.Info() : Local.Module_Experiment_Specifics_info11_unmanned); //"Operation""unmanned"
            }

            if (prefab.crew_reset.Length > 0)
            {
                var cs = new CrewSpecs(prefab.crew_reset);
                specs.Add(Local.Module_Experiment_Specifics_info12,
                    cs ? cs.Info() : Local.Module_Experiment_Specifics_info12_none); //"Reset""none"
            }
        ...
        {
            Message.Post(Lib.Color(Local.Module_Experiment_MultipleRunsMessage_title, Lib.Kolor.Orange, true),
                Local.Module_Experiment_MultipleRunsMessage.Format(title,
                    vesselName)); //"ALREADY RUNNING""Can't start " +  + " a second time on vessel " +
        }
