...
namespace Kerbalism.Modules
{
    public class Reliability : PartModule, ISpecifics, IModuleInfo, IPartCostModifier, IPartMassModifier
    {
        ...
        public double
            rated_radiation =
                0; // rad/h this part can sustain without taking any damage. Only effective with MTBF failures.

        [KSPField]
        public double
            radiation_decay_rate =
                1; // time to next failure is reduced by (rad/h - rated_radiation) * radiation_decay_rate per second
        ...
        [KSPField]
        public double rated_operation_duration = -1; // failure rate increases dramatically if this is exceeded
        ...
        public bool needMaintenance = false; // true when component is inspected and about to fail
        ...
        [KSPField(isPersistant = true)]
        public double operation_duration = 0.0; // failure rate increases dramatically if this is exceeded
        ...
        // status ui
        [KSPField(guiActive = true, guiActiveEditor = true, guiName = "_", groupName = "Reliability",
            groupDisplayName = "#KERBALISM_Group_Reliability")]
        //Reliability
        ...
        public override void OnStart(StartState state)
        {
            ...
            // setup ui
            Events["Inspect"].guiName =
                Local.Reliability_Inspect.Format("<b>" + title + "</b>"); //Lib.BuildString("Inspect <<1>>)
            Events["Repair"].guiName =
                Local.Reliability_Repair.Format("<b>" + title + "</b>"); //Lib.BuildString("Repair <<1>>)
        ...
        protected bool IgnitionCheck()
        {
            ...
            if (Time.time < Kerbalism.System.Kerbalism.gameLoadTime + 3)
                return false;
            ...
            bool fail = false;
            bool launchpad = vessel.situation == Vessel.Situations.PRELAUNCH ||
                             ignitions <= 1 && vessel.situation == Vessel.Situations.LANDED;
            ...
            if (turnon_failure_probability > 0)
            {
                ...
                q += Lib.Clamp(ignitions - 1, 0.0, 6.0) /
                     20.0; // subsequent ignition failures become less and less likely, reducing by up to 30%

                if (Lib.RandomDouble() <
                    (turnon_failure_probability * PreferencesReliability.Instance.ignitionFailureChance) / q)
                {
                    fail = true;
#if DEBUG_RELIABILITY
            ...
            if (rated_ignitions > 0)
            {
                ...
                if (ignitions > total_ignitions)
                {
                    ...
                    q /= (ignitions -
                          total_ignitions); // progressively increase the odds of a failure with every extra ignition
            ...
            if (fail)
            {
                ...
                if (Lib.RandomDouble() < fuelSystemFailureProbability)
                {
                    ...
                    FlightLogger.fetch?.LogEvent(
                        Local.FlightLogger_Destruction.Format(part.partInfo
                            .title)); // <<1>> fuel system leak caused destruction of the engine"
                }
                else
                {
                    FlightLogger.fetch?.LogEvent(
                        Local.FlightLogger_Ignition.Format(part.partInfo.title)); // <<1>> failure on ignition"
                }
            }
        ...
        public void Update()
        {
                ...
                if (part.IsPAWVisible())
                {
                    ...
                    if (broken)
                    {
                        Status = critical
                            ...
                            : Lib.Color(Local.Reliability_malfunction,
                                Lib.Kolor.Yellow); //"critical failure""malfunction"
                    }
                    else
                    {
                        if (PreferencesReliability.Instance.engineFailures &&
                            (rated_operation_duration > 0 || rated_ignitions > 0))
                        {
                            if (rated_operation_duration > 0)
                            {
                                double effective_duration = EffectiveDuration(quality, rated_operation_duration);
                                Status = Lib.BuildString(Local.Reliability_burnremaining, " ",
                                    Lib.HumanReadableDuration(Math.Max(0,
                                        effective_duration - operation_duration))); //"remaining burn:"
                            }
                            ...
                            if (rated_ignitions > 0)
                            {
                                ...
                                Status = Lib.BuildString(Status,
                                    ...
                                    Local.Reliability_ignitions, " ",
                                    Math.Max(0, effective_ignitions - ignitions).ToString()); //"ignitions:"
                            }
                        }
                        ...
                        if (rated_radiation > 0)
                        {
                            ...
                            if (rated < current)
                            {
                                Status = Lib.BuildString(Status, (string.IsNullOrEmpty(Status) ? "" : ", "),
                                    Lib.Color(Local.Reliability_takingradiationdamage,
                                        Lib.Kolor.Orange)); //"taking radiation damage"
                            }
                        }
                    }
                    ...
                    {
                        Events["Repair"].guiName =
                            Local.Reliability_Service.Format("<b>" + title + "</b>"); //Lib.BuildString("Service <<1>>")
                    }
                }
            ...
            else
            {
                ...
                if (part.IsPAWVisible())
                {
                    Events["Quality"].guiName =
                        Lib.StatusToggle(Local.Reliability_qualityinfo.Format("<b>" + title + "</b>"),
                            quality
                                ? Local.Reliability_qualityhigh
                                : Local.Reliability_qualitystandard); //Lib.BuildString(<<1>> quality")"high""standard"
        ...
        public void FixedUpdate()
        {
            ...
            if (!broken && mtbf > 0 && PreferencesReliability.Instance.mtbfFailures)
            {
                ...
                var decay = RadiationDecay(quality, vessel.KerbalismData().EnvRadiation,
                    Kerbalism.System.Kerbalism.elapsed_s, rated_radiation, radiation_decay_rate);
                next -= decay;
            }
        }
...
#endif
                    FlightLogger.fetch?.LogEvent(
                        Local.FlightLogger_MaterialFatigue
                            .Format(part.partInfo.title)); // <<1>> failed because of material fatigue"
                }
            }
        ...
        public static void BackgroundUpdate(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            Reliability reliability, double elapsed_s)
        {
            ...
            var decay = RadiationDecay(quality, rad, elapsed_s, reliability.rated_radiation,
                reliability.radiation_decay_rate);
            if (decay > 0)
            {
                next -= decay;
                Lib.Proto.Set(m, "next", next);
        ...
        [KSPEvent(guiActiveEditor = true, guiName = "_", active = true, groupName = "Reliability",
            groupDisplayName = "#KERBALISM_Group_Reliability")] //Reliability
        ...
        [KSPEvent(guiActiveUnfocused = true, unfocusedRange = 3.5f, guiName = "_", active = false,
            groupName = "Reliability", groupDisplayName = "#KERBALISM_Group_Reliability")] //Reliability
        ...
        public void Inspect()
        {
            ...
            if (rated_ignitions > 0 && ignitions >= Math.Ceiling(EffectiveIgnitions(quality, rated_ignitions) * 0.4))
                needMaintenance = true;
            if (rated_operation_duration > 0 &&
                operation_duration >= EffectiveDuration(quality, rated_operation_duration) * 0.4)
                needMaintenance = true;
        ...
        public void Break()
        {
            ...
            if (enforce_breakdown || vessel.KerbalismData().CrewCapacity > 0 ||
                Lib.RandomDouble() > PreferencesReliability.Instance.safeModeChance)
            {
                // flag as broken
                broken = true;
        ...
        public static void ProtoBreak(Vessel v, ProtoPartSnapshot p, ProtoPartModuleSnapshot m)
        {
            ...
            if (enforce_breakdown || v.KerbalismData().CrewCapacity > 0 ||
                Lib.RandomDouble() > PreferencesReliability.Instance.safeModeChance)
            {
                // flag as broken
        ...
        public static double RadiationDecay(bool quality, double rad, double elapsed_s, double rated_radiation,
            double radiation_decay_rate)
        {
            rad *= 3600.0;
        ...
        public Specifics Specs()
        {
            ...
            if (mtbf > 0)
                specs.Add(Local.Reliability_info4, Lib.HumanReadableDuration(EffectiveMTBF(false, mtbf))); //"MTBF"
            if (turnon_failure_probability > 0)
                specs.Add(Local.Reliability_info5,
                    Lib.HumanReadablePerc(turnon_failure_probability, "F1")); //"Ignition failures"
            if (rated_operation_duration > 0)
                specs.Add(Local.Reliability_info6,
                    Lib.HumanReadableDuration(EffectiveDuration(false,
                        rated_operation_duration))); //"Rated burn duration"
            if (rated_ignitions > 0)
                specs.Add(Local.Reliability_info7,
                    EffectiveIgnitions(false, rated_ignitions).ToString()); //"Rated ignitions"
            if (mtbf > 0 && rated_radiation > 0)
                specs.Add(Local.Reliability_info8,
                    Lib.HumanReadableRadiation(rated_radiation / 3600.0)); //"Radiation rating"

            specs.Add(string.Empty);
            specs.Add("<color=#00ffff>" + Local.Reliability_info9 + "</color>"); //High quality
            if (extra_cost > double.Epsilon)
                specs.Add(Local.Reliability_info10,
                    Lib.HumanReadableCost(extra_cost * part.partInfo.cost)); //"Extra cost"
            if (extra_mass > double.Epsilon)
                specs.Add(Local.Reliability_info11,
                    Lib.HumanReadableMass(extra_mass * part.partInfo.partPrefab.mass)); //"Extra mass"
            if (mtbf > 0)
                specs.Add(Local.Reliability_info4, Lib.HumanReadableDuration(EffectiveMTBF(true, mtbf))); //"MTBF"
            if (turnon_failure_probability > 0)
                specs.Add(Local.Reliability_info5,
                    Lib.HumanReadablePerc(turnon_failure_probability / Settings.QualityScale,
                        "F1")); //"Ignition failures"
            if (rated_operation_duration > 0)
                specs.Add(Local.Reliability_info6,
                    Lib.HumanReadableDuration(EffectiveDuration(true,
                        rated_operation_duration))); //"Rated burn duration"
            if (rated_ignitions > 0)
                specs.Add(Local.Reliability_info7,
                    EffectiveIgnitions(true, rated_ignitions).ToString()); //"Rated ignitions"
            if (mtbf > 0 && rated_radiation > 0)
                specs.Add(Local.Reliability_info8,
                    Lib.HumanReadableRadiation(Settings.QualityScale * rated_radiation / 3600.0)); //"Radiation rating"
        ...
        {
            return Lib.BuildString(title, " ", Local.Reliability_Reliability);
        } //Reliability
        ...
        static void Broken_msg(Vessel v, string title, bool critical)
        {
            if (v.KerbalismData().cfg_malfunction)
            {
                if (!critical)
                {
                    ...
                    (
                        Severity.warning,
                        Local.Reliability_MessagePost24.Format("<b>" + title + "</b>",
                            "<b>" + v.vesselName + "</b>"), //Lib.BuildString(<<1>> malfunctioned on <<2>>)
                ...
                else
                {
                    ...
                    (
                        Severity.danger,
                        Local.Reliability_MessagePost26.Format("<b>" + title + "</b>",
                            "<b>" + v.vesselName + "</b>"), //Lib.BuildString(<<1>> failed on <<2>>)
        ...
        static void Safemode_msg(Vessel v, string title)
        {
            Message.Post
            (
                Local.Reliability_MessagePost28.Format("<b>" + title + "</b>",
                    "<b>" + v.vesselName + "</b>"), //Lib.BuildString("There has been a problem with <<1>> on <<2>>)
        ...
        public static void CauseMalfunction(Vessel v)
        {
            ...
            {
                // choose a module at random
                var modules = Lib.FindModules(v.protoVessel, "Reliability")
                    .FindAll(k => !Lib.Proto.GetBool(k, "broken"));
                if (modules.Count == 0) return;
                var m = modules[Lib.RandomInt(modules.Count)];
