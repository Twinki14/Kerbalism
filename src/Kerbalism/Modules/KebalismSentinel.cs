...
namespace Kerbalism.Modules
{
    public class KerbalismSentinel : SentinelModule, IContractObjectiveModule
    {
        ...
        [KSPField(isPersistant = true)] public bool isTrackingEnabled;
        ...
        public static void BackgroundUpdate(Vessel v, ProtoPartModuleSnapshot m, KerbalismSentinel prefab,
            VesselData vd, ResourceInfo ec, double elapsed_s)
        {
        ...
        public static void ApplyHarmonyPatches(Harmony harmonyInstance)
        {
            ...
            MethodInfo startTracking = typeof(SentinelModule).GetMethod("StartTracking",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            MethodInfo startTrackingPostfix = typeof(KerbalismSentinel).GetMethod("StartTrackingPostfix",
                BindingFlags.Static | BindingFlags.NonPublic);
            harmonyInstance.Patch(startTracking, null, new HarmonyMethod(startTrackingPostfix));

            MethodInfo stopTracking = typeof(SentinelModule).GetMethod("StopTracking",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            MethodInfo stopTrackingPostfix = typeof(KerbalismSentinel).GetMethod("StopTrackingPostfix",
                BindingFlags.Static | BindingFlags.NonPublic);
            harmonyInstance.Patch(stopTracking, null, new HarmonyMethod(stopTrackingPostfix));

            MethodInfo fixedUpdate = typeof(SentinelModule).GetMethod("FixedUpdate",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            MethodInfo fixedUpdatePrefix =
                typeof(KerbalismSentinel).GetMethod("FixedUpdatePrefix", BindingFlags.Static | BindingFlags.NonPublic);
            harmonyInstance.Patch(fixedUpdate, new HarmonyMethod(fixedUpdatePrefix));

            MethodInfo telescopeCanActivate = typeof(SentinelModule).GetMethod("TelescopeCanActivate",
                BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic);
            MethodInfo telescopeCanActivatePrefix = typeof(KerbalismSentinel).GetMethod("TelescopeCanActivatePrefix",
                BindingFlags.Static | BindingFlags.NonPublic);
        ...
        private static bool FixedUpdatePrefix(KerbalismSentinel __instance)
        {
            if (__instance.isTrackingEnabled)
            {
                ...
                ec.Consume(__instance.ec_rate * Kerbalism.System.Kerbalism.elapsed_s, ResourceBroker.Scanner);
        ...
        private static bool TelescopeCanActivatePrefix(KerbalismSentinel __instance, ref bool __result)
        {
            if (__instance.vessel.orbit.referenceBody != Planetarium.fetch.Sun)
            {
                ...
                ScreenMessages.PostScreenMessage(msg, SentinelUtilities.CalculateReadDuration(msg),
        ...
        public override string GetInfo()
        {
            ...
            if (ec_rate > 0.0)
                specs.Add(Local.Module_Experiment_Specifics_info9, Lib.HumanOrSIRate(ec_rate, Lib.ECResID)); //"EC"
            if (comms_rate > 0.0)
                specs.Add(Local.Module_Experiment_Specifics_info2,
                    Lib.HumanReadableDataRate(comms_rate)); // "Data rate"
