...
namespace Kerbalism.Modules
{
    ...
    public class Sensor : PartModule, ISpecifics
    {
        ...
        // status
        [KSPField(guiActive = true, guiName = "_", groupName = "Sensors", groupDisplayName = "#KERBALISM_Group_Sensors",
            groupStartCollapsed = true)]
        public string Status; //Sensors
            ...
            switch (type)
            {
                ...
                case "gravioli":
                    return vd.EnvGravioli < 0.33 ? Local.Sensor_shorttextinfo1 :
                        ...
                        Local.Sensor_shorttextinfo3; //"nothing here""almost one""WOW!"
            }
        ...
        public static string Telemetry_tooltip(Vessel v, VesselData vd, string type)
        {
            switch (type)
            {
                case "temperature":
                    ...
                    (
                        "<align=left />",
                        String.Format("{0,-14}\t<b>{1}</b>\n", Local.Sensor_solarflux,
                            Lib.HumanReadableFlux(vd.EnvSolarFluxTotal)), //"solar flux"
                        String.Format("{0,-14}\t<b>{1}</b>\n", Local.Sensor_albedoflux,
                            Lib.HumanReadableFlux(vd.EnvAlbedoFlux)), //"albedo flux"
                        String.Format("{0,-14}\t<b>{1}</b>", Local.Sensor_bodyflux,
                            Lib.HumanReadableFlux(vd.EnvBodyFlux)) //"body flux"
                    );
                ...
                case "pressure":
                    return vd.EnvUnderwater
                        ...
                        : vd.EnvInAtmosphere
                            ? Local.Sensor_insideatmosphere.Format(vd.EnvBreathable
                                ? Local.Sensor_breathable
                                : Local
                                    .Sensor_notbreathable) //"breathable""not breathable"                  //Lib.BuildString("inside <b>atmosphere</b> (", vd.EnvBreathable ? "breathable" : "not breathable", ")")
                ...
                case "gravioli":
                    return Lib.BuildString
                    (
                        Local.Sensor_Graviolidetection + " <b>" + vd.EnvGravioli.ToString("F2") +
