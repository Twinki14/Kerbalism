...
namespace Kerbalism.Modules
{
    public class PassiveShield : PartModule, IPartMassModifier, IKerbalismModule
    {
        ...
        [KSPField]
        public string
            title = Local.PassiveShield_Sandbags; //"Sandbags"              // GUI name of the status action in the PAW

        [KSPField]
        public string
            engageActionTitle = Local.PassiveShield_fill; //"fill"      // what the deploy action should be called

        [KSPField]
        public string
            disengageActionTitle = Local.PassiveShield_empty; //"empty"  // what the empty action should be called

        [KSPField]
        public string
            disabledTitle =
                Local.PassiveShield_stowed; // "stowed"       // what to display in the status text while not deployed
        ...
        public void Update()
        {
            ...
            // update ui
            Status = deployed
                ? Lib.BuildString(Local.PassiveShield_absorbing, " ", Lib.HumanReadableRadiation(Math.Abs(radiation)))
                : disabledTitle; //"absorbing
