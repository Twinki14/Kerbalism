...
namespace Kerbalism.Modules
{
    ...
    public class SolarPanelFixer : PartModule
    {
        ...
        [KSPField] public string EcUIUnit = string.Empty;
        ...
        [KSPField(guiActive = true, guiActiveEditor = false,
            guiName = "#KERBALISM_SolarPanelFixer_Solarpanel")]
        //Solar panel
        public string panelStatus = string.Empty;

        [KSPField(isPersistant = true, guiActive = false, guiActiveEditor = true,
            guiName = "#KERBALISM_SolarPanelFixer_Solarpaneloutput")] //Solar panel output
        [UI_Toggle(enabledText = "#KERBALISM_SolarPanelFixer_simulated",
            disabledText = "#KERBALISM_SolarPanelFixer_ignored")]
        //<color=#00ff00>simulated</color>""<color=#ffff00>ignored</color>
        ...
        [KSPField(isPersistant = true)] public PanelState state;

        /// <summary>tracked star/sun body index</summary>
        [KSPField(isPersistant = true)] public int trackedSunIndex = 0;

        /// <summary>has the player manually selected the star to be tracked ?</summary>
        [KSPField(isPersistant = true)] private bool manualTracking = false;
        ...
        /// </summary>
        [KSPField(isPersistant = true)] public FloatCurve timeEfficCurve;
        ...
        [KSPField(isPersistant = true)] public double launchUT = -1.0;
        ...
        [KSPField] public double currentOutput;
        ...
        [KSPEvent(active = true, guiActive = true,
            guiName = "#KERBALISM_SolarPanelFixer_Selecttrackedstar")] //Select tracked star
        public void ManualTracking()
        {
            // Assemble the buttons
            DialogGUIBase[] options = new DialogGUIBase[Sim.suns.Count + 1];
            options[0] =
                new DialogGUIButton(Local.SolarPanelFixer_Automatic, () => { manualTracking = false; },
                    true); //"Automatic"
        ...
        public override void OnStart(StartState startState)
        {
            ...
            Events["ManualTracking"].guiActive = state == PanelState.Extended || state == PanelState.ExtendedFixed ||
        ...
        public void Update()
        {
            ...
            if (Events["ManualTracking"].active && (state == PanelState.Extended || state == PanelState.ExtendedFixed ||
                                                    state == PanelState.Static))
            {
                Events["ManualTracking"].guiActive = true;
                Events["ManualTracking"].guiName = Lib.BuildString(Local.SolarPanelFixer_Trackedstar + " ",
                    manualTracking ? ": " : Local.SolarPanelFixer_AutoTrack,
                    FlightGlobals.Bodies[trackedSunIndex].bodyDisplayName.Replace("^N", "")); //"Tracked star"[Auto] : "
            }
            else
            {
            ...
            switch (exposureState)
            {
                ...
                case ExposureState.OccludedTerrain:
                    panelStatus =
                        "<color=#ff2222>" + Local.SolarPanelFixer_occludedbyterrain + "</color>"; //occluded by terrain
                    addRate = true;
                    break;
                case ExposureState.OccludedPart:
                    panelStatus = Lib.BuildString("<color=#ff2222>",
                        Local.SolarPanelFixer_occludedby.Format(mainOccludingPart), "</color>"); //occluded by
                    addRate = true;
                    break;
                case ExposureState.BadOrientation:
                    panelStatus =
                        "<color=#ff2222>" + Local.SolarPanelFixer_badorientation + "</color>"; //bad orientation
                    addRate = true;
                    break;
                case ExposureState.Disabled:
                    switch (state)
                    {
                        case PanelState.Retracted:
                            panelStatus = Local.SolarPanelFixer_retracted;
                            break; //"retracted"
                        case PanelState.Extending:
                            panelStatus = Local.SolarPanelFixer_extending;
                            break; //"extending"
                        case PanelState.Retracting:
                            panelStatus = Local.SolarPanelFixer_retracting;
                            break; //"retracting"
                        case PanelState.Broken:
                            panelStatus = Local.SolarPanelFixer_broken;
                            break; //"broken"
                        case PanelState.Failure:
                            panelStatus = Local.SolarPanelFixer_failure;
                            break; //"failure"
                        case PanelState.Unknown:
                            panelStatus = Local.SolarPanelFixer_invalidstate;
                            break; //"invalid state"
                    }
        ...
        public void FixedUpdate()
        {
            ...
            {
                state = newState;
                if (Lib.IsEditor() && (newState == PanelState.Extended || newState == PanelState.ExtendedFixed ||
                                       newState == PanelState.Retracted))
                    Lib.RefreshPlanner();
            }
            ...
            else
            {
                ...
                foreach (VesselData.SunInfo sunInfo in vd.EnvSunsInfo)
                {
                    ...
                    else
                    {
                        ...
                        sunOccludedFactor = SolarPanel.GetOccludedFactor(sunInfo.Direction, out occludingPart,
                            sunInfo != trackedSunInfo);
            ...
            if (timeEfficCurve?.Curve.keys.Length > 1)
                wearFactor =
                    Lib.Clamp(timeEfficCurve.Evaluate((float) ((Planetarium.GetUniversalTime() - launchUT) / 3600.0)),
                        0.0, 1.0);
            ...
            // produce EC
            ec.Produce(currentOutput * Kerbalism.System.Kerbalism.elapsed_s, ResourceBroker.SolarPanel);
            UnityEngine.Profiling.Profiler.EndSample();
        }

        public static void BackgroundUpdate(Vessel v, ProtoPartModuleSnapshot m, SolarPanelFixer prefab, VesselData vd,
            ResourceInfo ec, double elapsed_s)
        {
            ...
            if (m.moduleValues.HasNode("timeEfficCurve"))
            {
                ...
                efficiencyFactor *=
                    Lib.Clamp(teCurve.Evaluate((float) ((Planetarium.GetUniversalTime() - launchUT) / 3600.0)), 0.0,
                        1.0);
            }
        ...
        public bool GetSolarPanelModule()
        {
            ...
            foreach (PartModule pm in part.Modules)
            {
                ...
                switch (pm.moduleName)
                {
                    case "ModuleCurvedSolarPanel":
                        SolarPanel = new NFSCurvedPanel();
                        break;
                    case "SSTUSolarPanelStatic":
                        SolarPanel = new SSTUStaticPanel();
                        break;
                    case "SSTUSolarPanelDeployable":
                        SolarPanel = new SSTUVeryComplexPanel();
                        break;
                    case "SSTUModularPart":
                        SolarPanel = new SSTUVeryComplexPanel();
                        break;
                    case "ModuleROSolar":
                        SolarPanel = new ROConfigurablePanel();
                        break;
                    case "KopernicusSolarPanel":
                        Lib.Log(
                            "Part '" + part.partInfo.title +
                            "' use the KopernicusSolarPanel module, please remove it from your config. Kerbalism has it's own support for Kopernicus",
            ...
            if (SolarPanel == null)
            {
                Lib.Log("Could not find a supported solar panel module, disabling SolarPanelFixer module...",
                    Lib.LogLevel.Warning);
                enabled = isEnabled = moduleIsEnabled = false;
                return false;
            }
        ...
        public static void ProtoToggleState(SolarPanelFixer prefab, ProtoPartModuleSnapshot protoModule,
            PanelState currentState)
        {
            switch (currentState)
            {
                case PanelState.Retracted:
                    ...
                    {
                        SetProtoState(protoModule, PanelState.Extended);
                        return;
                    }

                    SetProtoState(protoModule, PanelState.ExtendedFixed);
                    return;
                case PanelState.Extended:
                    SetProtoState(protoModule, PanelState.Retracted);
                    return;
            }
        }
        ...
        public abstract class SupportedPanel
        {
            ...
            {
                return new FloatCurve(new Keyframe[] {new Keyframe(0f, 1f)});
            }
            ...
            public virtual bool IsRetractable()
            {
                return false;
            }
        ...
        {
            private Transform
                sunCatcherPosition; // middle point of the panel surface (usually). Use only position, panel surface direction depend on the pivot transform, even for static panels.

            private Transform
                sunCatcherPivot; // If it's a tracking panel, "up" is the pivot axis and "position" is the pivot position. In any case "forward" is the panel surface normal.
            ...
            public override bool OnStart(bool initialized, ref double nominalRate)
            {
                ...
                if (sunCatcherPosition == null)
                {
                    Lib.Log("Could not find suncatcher transform `{0}` in part `{1}`", Lib.LogLevel.Error,
                        panelModule.secondaryTransformName, panelModule.part.name);
                    return false;
                }
            ...
            public override FloatCurve GetTimeCurve()
            {
                if (panelModule.timeEfficCurve?.Curve.keys.Length > 1)
                {
                    ...
                    foreach (Keyframe key in panelModule.timeEfficCurve.Curve.keys)
                        timeCurve.Add(key.time * 24f, key.value, key.inTangent * (1f / 24f),
                            key.outTangent * (1f / 24f));
                    return timeCurve;
                }
            ...
            public override double GetOccludedFactor(Vector3d sunDir, out string occludingPart, bool analytic = false)
            {
                ...
                if (analytic)
                {
                    ...
                    Physics.Raycast(sunCatcherPosition.position + (sunDir * panelModule.raycastOffset), sunDir,
                        out raycastHit, 10000f);
                }
                else
                {
            ...
            public override double GetCosineFactor(Vector3d sunDir, bool analytic = false)
            {
                ...
                switch (panelModule.panelType)
                {
                    ...
                    case ModuleDeployableSolarPanel.PanelType.CYLINDRICAL:
                        return Math.Max(
                            (1.0 - Math.Abs(Vector3d.Dot(sunDir, panelModule.trackingDotTransform.forward))) *
                            (1.0 / Math.PI), 0.0);
            ...
            public override bool IsRetractable()
            {
        ...
        private class NFSCurvedPanel : SupportedPanel<PartModule>
        {
            ...
            public override PanelState GetState()
            {
                ...
                ModuleDeployablePart.DeployState state =
                    (ModuleDeployablePart.DeployState) Enum.Parse(enumtype, stateStr);
            ...
            public override void Extend()
            {
                Lib.ReflectionCall(panelModule, "DeployPanels");
            }

            public override void Retract()
            {
                Lib.ReflectionCall(panelModule, "RetractPanels");
            }
        ...
        private class SSTUVeryComplexPanel : SupportedPanel<PartModule>
        {
            ...
            private Func<string>
                getAnimationState; // delegate for the AnimationModule.persistentData property (string of the animState struct)
            ...
            private class SSTUPanelData
            {
                ...
                {
                    public object
                        objectRef; // reference to the "SuncatcherData" class instance, used to get the raycast hit (direct ref to the RaycastHit doesn't work)
                ...
                public Vector3 SuncatcherAxisVector(int index) =>
                    GetDirection(suncatchers[index].transform, suncatchers[index].axis);

                public RaycastHit SuncatcherHit(int index) =>
                    Lib.ReflectionValue<RaycastHit>(suncatchers[index].objectRef, "hitData");
                ...
                public static Axis ParseSSTUAxis(object sstuAxis)
                {
                    return (Axis) Enum.Parse(typeof(Axis), sstuAxis.ToString());
                }
            ...
            public override bool OnStart(bool initialized, ref double nominalRate)
            {
                ...
                try
                {
                    ...
                    getAnimationState = (Func<string>) Delegate.CreateDelegate(typeof(Func<string>),
                        animationModuleSSTU, prop.GetGetMethod());
                    ...
                    object[] panelDataArray =
                        Lib.ReflectionValue<object[]>(solarModuleSSTU,
                            "panelData"); // retrieve the PanelData class array that contain suncatchers and pivots data arrays
                    foreach (object panel in panelDataArray)
                    {
                        object[] suncatchers =
                            Lib.ReflectionValue<object[]>(panel,
                                "suncatchers"); // retrieve the SuncatcherData class array
                        object[] pivots =
                            Lib.ReflectionValue<object[]>(panel, "pivots"); // retrieve the SolarPivotData class array
                        ...
                        for (int i = 0; i < suncatchersCount; i++)
                        {
                            ...
                            if (Lib.IsFlight())
                                Lib.ReflectionValue(suncatcher, "resourceRate",
                                    0f); // actually prevent SSTU modules from generating EC, but not in the editor
                            panelData.suncatchers[i] = new SSTUPanelData.SSTUSunCatcher();
                            panelData.suncatchers[i].objectRef =
                                suncatcher; // keep a reference to the original suncatcher instance, for raycast hit acquisition
                            panelData.suncatchers[i].transform =
                                Lib.ReflectionValue<Transform>(suncatcher, "suncatcher"); // get suncatcher transform
                            panelData.suncatchers[i].axis =
                                SSTUPanelData.ParseSSTUAxis(Lib.ReflectionValue<object>(suncatcher,
                                    "suncatcherAxis")); // get suncatcher axis
                        }
                        ...
                        {
                            case 0:
                                trackingType = TrackingType.Fixed;
                                break;
                            case 1:
                                trackingType = TrackingType.SinglePivot;
                                panelData.pivot = Lib.ReflectionValue<Transform>(pivots[0], "pivot");
                                panelData.pivotAxis =
                                    SSTUPanelData.ParseSSTUAxis(
                                        Lib.ReflectionValue<object>(pivots[0], "pivotRotationAxis"));
                                break;
                            case 2:
                                trackingType = TrackingType.DoublePivot;
                                break;
                            default: continue;
                        }
                    ...
                    {
                        case "SSTUModularPart":
                            panelModule.Fields["solarPanelStatus"].guiActive = false;
                            break;
                        case "SSTUSolarPanelDeployable":
                ...
                {
                    Lib.Log(
                        "SolarPanelFixer : exception while getting SSTUModularPart/SSTUSolarPanelDeployable solar panel data : " +
                        ex.Message);
                    return false;
                }
#endif
                    ...
                    for (int i = 0; i < panel.SuncatcherCount; i++)
                    {
                        ...
                        switch (trackingType)
                        {
                            ...
                            case TrackingType.SinglePivot:
                                cosineFactor +=
                                    Math.Cos(1.57079632679 - Math.Acos(Vector3d.Dot(sunDir, panel.PivotAxisVector)));
                                continue;
            ...
            public override double GetOccludedFactor(Vector3d sunDir, out string occludingPart, bool analytic = false)
            {
                ...
                foreach (SSTUPanelData panel in panels)
                {
                    ...
                    for (int i = 0; i < panel.SuncatcherCount; i++)
                    {
                        RaycastHit raycastHit;
                        if (analytic)
                            Physics.Raycast(panel.SuncatcherPosition(i) + (sunDir * 0.25), sunDir, out raycastHit,
                                10000f);
                        else
                        ...
                        {
                            occludingFactor +=
                                1.0; // in case of multiple panels per part, it is perfectly valid for panels to occlude themselves so we don't do the usual check
                            Part blockingPart = Part.GetComponentUpwards<Part>(raycastHit.transform.gameObject);
                            if (occludingPart == null &&
                                blockingPart != null) // don't update if occlusion is from multiple parts
                                occludingPart = blockingPart.partInfo.title;
                        }
                    }
                }
            ...
            public override bool IsTracking =>
                trackingType == TrackingType.SinglePivot || trackingType == TrackingType.DoublePivot;
    ...
    class SolarDebugDrawer : MonoBehaviour
    {
            ...
            while (true)
            {
                ...
                try
                {
                    ...
                    Matrix4x4 projectionMatrix =
                        Matrix4x4.Perspective(cam.fieldOfView, cam.aspect, cam.nearClipPlane, float.MaxValue);
        ...
        private static void DrawTransform(Vector3 position, Vector3 up, Vector3 right, Vector3 forward,
            float scale = 1.0f)
        {
