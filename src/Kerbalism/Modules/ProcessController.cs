...
using UnityEngine;
using Process = Kerbalism.Profile.Process;
...
namespace Kerbalism.Modules
{
    public class ProcessController : PartModule, IModuleInfo, IAnimatedModule, ISpecifics, IConfigurable
    {
        ...
        public override void OnLoad(ConfigNode node)
        {
            ...
            {
                Process process = Profile.Profile.processes.Find(x => x.modifiers.Contains(resource));
        ...
        public void Update()
        {
            ...
            // update rmb ui
            Events["Toggle"].guiName = Lib.StatusToggle(title,
        ...
        [KSPEvent(guiActive = true, guiActiveEditor = true, guiName = "_", active = true, groupName = "Processes",
            groupDisplayName = "#KERBALISM_Group_Processes")] //Processes
        public void Toggle()
        {
            SetRunning(!running);
        }

        [KSPEvent(guiActive = true, guiActiveEditor = true, guiName = "#KERBALISM_ProcessController_Dump",
            active = true, groupName = "Processes",
            groupDisplayName = "#KERBALISM_Group_Processes")] //"Dump""Processes"
        ...
        public Specifics Specs()
        {
            ...
            Process process = Profile.Profile.processes.Find(k => k.modifiers.Contains(resource));
            if (process != null)
            {
                foreach (KeyValuePair<string, double> pair in process.inputs)
                {
                    if (!process.modifiers.Contains(pair.Key))
                        specs.Add(Lib.GetResourceDisplayName(pair.Key),
                            Lib.BuildString(" <color=#ffaa00>",
                                Lib.HumanOrSIRate(pair.Value * capacity, pair.Key.GetHashCode()), "</color>"));
                    else
                        specs.Add(Local.ProcessController_info1,
                            Lib.HumanReadableDuration(0.5 / pair.Value)); //"Half-life"
                }

                foreach (KeyValuePair<string, double> pair in process.outputs)
                {
                    specs.Add(Lib.GetResourceDisplayName(pair.Key),
                        Lib.BuildString(" <color=#00ff00>",
                            Lib.HumanOrSIRate(pair.Value * capacity, pair.Key.GetHashCode()), "</color>"));
                }
            }
