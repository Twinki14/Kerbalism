...
namespace Kerbalism.Modules
{
    public class Greenhouse : PartModule, IModuleInfo, ISpecifics, IContractObjectiveModule, IConfigurable
    {
        ...
        [KSPField]
        public double pressure_tolerance; // minimum pressure required for growth, in sea level atmospheres (optional)
        ...
        // rmb ui status
        [KSPField(guiActive = true, guiName = "#KERBALISM_Greenhouse_status_natural", groupName = "Greenhouse",
            groupDisplayName = "#KERBALISM_Group_Greenhouse")]
        //Greenhouse
        public string status_natural; // natural lighting

        [KSPField(guiActive = true, guiName = "#KERBALISM_Greenhouse_status_artificial", groupName = "Greenhouse",
            groupDisplayName = "#KERBALISM_Group_Greenhouse")]
        //Greenhouse
        public string status_artificial; // artificial lighting

        [KSPField(guiActive = true, guiName = "#KERBALISM_Greenhouse_status_tta", groupName = "Greenhouse",
            groupDisplayName = "#KERBALISM_Group_Greenhouse")]
        //Greenhouse
        ...
        // other data
        UnityEngine.Renderer lamps_rdr;
        ...
        public void Update()
        {
            ...
            if (lamps_rdr != null)
            {
                float intensity = Lib.IsFlight()
                    ? (active ? (float) (artificial / light_tolerance) : 0.0f)
                    : (active ? 1.0f : 0.0f);
            ...
            if (Lib.IsFlight())
            {
                ...
                {
                    string status = issue.Length > 0 ? Lib.BuildString("<color=yellow>", issue, "</color>") :
                        growth > 0.99 ? Local.TELEMETRY_readytoharvest :
                        Local.TELEMETRY_growing; //"ready to harvest""growing"
                    Events["Toggle"].guiName = Lib.StatusToggle(Local.Greenhouse_Greenhouse,
                        active ? status : Local.Greenhouse_disabled); //"Greenhouse""disabled"
            ...
            else if (part.IsPAWVisible())
            {
                // update ui
                Events["Toggle"].guiName = Lib.StatusToggle(Local.Greenhouse_Greenhouse,
                    active ? Local.Greenhouse_enabled : Local.Greenhouse_disabled); //"Greenhouse""enabled""disabled"
            }
        }
        ...
        public void FixedUpdate()
        {
            ...
            if (active && growth < 0.99)
            {
                ...
                if (artificial > double.Epsilon)
                    ec.Consume(ec_rate * (artificial / light_tolerance) * Kerbalism.System.Kerbalism.elapsed_s,
                ...
                foreach (ModuleResource input in resHandler.inputResources)
                {
                    ...
                    if (WACO2 && input.name == "WasteAtmosphere")
                        recipe.AddInput(input.name,
                            vd.EnvBreathable ? 0.0 : input.rate * Kerbalism.System.Kerbalism.elapsed_s,
                            "CarbonDioxide");
                    ...
                    else recipe.AddInput(input.name, input.rate * Kerbalism.System.Kerbalism.elapsed_s);
                }
                ...
                foreach (ModuleResource output in resHandler.outputResources)
                {
                    ...
                    if (output.name == "Oxygen")
                        recipe.AddOutput(output.name,
                            vd.EnvBreathable ? 0.0 : output.rate * Kerbalism.System.Kerbalism.elapsed_s, true);
                    else recipe.AddOutput(output.name, output.rate * Kerbalism.System.Kerbalism.elapsed_s, true);
                }
                ...
                bool radiation = radiation_tolerance <= double.Epsilon ||
                                 (1.0 - vd.Shielding) * vd.EnvHabitatRadiation < radiation_tolerance;
                ...
                foreach (ModuleResource input in resHandler.inputResources)
                {
                    ...
                    if (input.name == "WasteAtmosphere" || input.name == "CarbonDioxide")
                    {
                        ...
                        if (WACO2)
                        {
                            if (resources.GetResource(vessel, "WasteAtmosphere").Amount <= double.Epsilon &&
                                resources.GetResource(vessel, "CarbonDioxide").Amount <= double.Epsilon)
                            {
                                inputs = false;
                ...
                {
                    // increase growth
                    growth += crop_rate * Kerbalism.System.Kerbalism.elapsed_s;
                    ...
                    {
                        Message.Post(
                            Local.harvestedready_msg.Format("<b>" + vessel.vesselName +
                                                            "</b>")); //Lib.BuildString("On <<1>> the crop is ready to be harvested")
                        growth = 1.0;
                    }
                }
            ...
            VesselData vd, VesselResources resources, double elapsed_s)
        {
            ...
            if (active && growth < 0.99)
            {
                ...
                // execute recipe
                ResourceRecipe recipe = new ResourceRecipe(ResourceBroker.Greenhouse);
                foreach (ModuleResource input in
                         g.resHandler.inputResources) //recipe.Input(input.name, input.rate * elapsed_s);
                {
                    // WasteAtmosphere is primary combined input
                ...
                bool radiation = g.radiation_tolerance <= 0 ||
                                 vd.EnvRadiation * (1.0 - vd.Shielding) < g.radiation_tolerance;
                ...
                foreach (ModuleResource input in g.resHandler.inputResources)
                {
                    ...
                    if (input.name == "WasteAtmosphere" || input.name == "CarbonDioxide")
                    {
                        ...
                        if (g.WACO2)
                        {
                            if (resources.GetResource(v, "WasteAtmosphere").Amount <= double.Epsilon &&
                                resources.GetResource(v, "CarbonDioxide").Amount <= double.Epsilon)
                            {
                                inputs = false;
                                missing_res = "CarbonDioxide";
                                break;
                ...
                if (lighting && pressure && radiation && inputs)
                {
                    ...
                    {
                        Message.Post(
                            Local.harvestedready_msg.Format("<b>" + v.vesselName +
                                                            "</b>")); //Lib.BuildString("On <<1>> the crop is ready to be harvested")
                        growth = 1.0;
                    }
                }
                ...
                string issue =
                    !inputs
                        ? Lib.BuildString(
                            Local.Greenhouse_resoucesmissing.Format(missing_res)) //"missing ", missing_res
                        : !lighting
                            ? Local.Greenhouse_issue1 //"insufficient lighting"
                            : !pressure
                                ? Local.Greenhouse_issue2 //"insufficient pressure"
                                : !radiation
                                    ? Local.Greenhouse_issue3 //"excessive radiation"
                                    : string.Empty;
        ...
        [KSPEvent(guiActive = true, guiActiveEditor = true, guiName = "_", groupName = "Greenhouse",
            groupDisplayName = "#KERBALISM_Group_Greenhouse")] //Greenhouse
        public void Toggle()
        {
        ...
        [KSPEvent(guiActive = true, guiActiveUnfocused = true, guiName = "#KERBALISM_Greenhouse_Harvest",
            active = false, groupName = "Greenhouse", groupDisplayName = "#KERBALISM_Group_Greenhouse")] //Greenhouse
        public void Harvest()
        {
            ...
            Message.Post(Lib.BuildString(
                Local.Greenhouse_msg_1.Format("<color=ffffff>" + vessel.vesselName + "</color> "),
                Local.Greenhouse_msg_2.Format("<color=ffffff>" + crop_size.ToString("F0") + " " +
                                              Lib.GetResourceDisplayName(crop_resource) +
                                              "</color>"))); //"On <<1>>""harvest produced <<1>>",

            // record first harvest
            if (!Lib.Landed(vessel)) DB.landmarks.space_harvest = true;
        }

        [KSPEvent(guiActive = true, guiActiveUnfocused = true, guiName = "#KERBALISM_Greenhouse_EmergencyHarvest",
            active = false, groupName = "Greenhouse", groupDisplayName = "#KERBALISM_Group_Greenhouse")] //Greenhouse
        public void EmergencyHarvest()
        {
            ...
            Message.Post(Lib.BuildString(
                Local.Greenhouse_msg_1.Format("<color=ffffff>" + vessel.vesselName + "</color> "),
                Local.Greenhouse_msg_3.Format(" <color=ffffff>" + reduced_harvest.ToString("F0") + " " + crop_resource +
                                              "</color>"))); //"On <<1>>""emergency harvest produced"
        ...
        // action groups
        [KSPAction("#KERBALISM_Greenhouse_Action")]
        ...
        public override string GetInfo()
        {
            if (!isConfigurable)
                return
                    Specs().Info(Local
                        .Greenhouse_desc); //"Grow crops in space and on the surface of celestial bodies, even far from the sun."
            else
                return string.Empty;
        }
        ...
        public Specifics Specs()
        {
            ...
            specs.Add(Local.Greenhouse_info1,
                Lib.HumanReadableAmount(crop_size, " " + Lib.GetResourceDisplayName(crop_resource))); //"Harvest size"
            ...
            if (pressure_tolerance > double.Epsilon)
                specs.Add(Local.Greenhouse_info4,
                    Lib.HumanReadablePressure(Sim.PressureAtSeaLevel() * pressure_tolerance)); //"Pressure tolerance"
            if (radiation_tolerance > double.Epsilon)
                specs.Add(Local.Greenhouse_info5,
                    Lib.HumanReadableRadiation(radiation_tolerance)); //"Radiation tolerance"
            ...
            foreach (ModuleResource input in resHandler.inputResources)
            {
                ...
                if (WACO2 && (input.name == "WasteAtmosphere" || input.name == "CarbonDioxide"))
                {
                    ...
                    specs.Add(Local.Greenhouse_CarbonDioxide,
                        Lib.BuildString("<color=#ffaa00>",
                            Lib.HumanOrSIRate(input.rate + sec.rate, "CarbonDioxide".GetHashCode()),
                            " </color>")); //"CarbonDioxide"
                    specs.Add(Local
                        .Greenhouse_CarbonDioxide_desc); //"Crops can also use the CO2 in the atmosphere without a scrubber."
                    dis_WACO2 = true;
                }
                else
                    specs.Add(Lib.GetResourceDisplayName(input.name),
                        Lib.BuildString("<color=#ffaa00>", Lib.HumanOrSIRate(input.rate, input.id), "</color>"));
            }
            ...
            foreach (ModuleResource output in resHandler.outputResources)
            {
                specs.Add(Lib.GetResourceDisplayName(output.name),
                    Lib.BuildString("<color=#00ff00>", Lib.HumanOrSIRate(output.rate, output.id), "</color>"));
            }
        ...
        {
            return "<size=1><color=#00000000>00</color></size>Greenhouse";
        } // attempt to display at the top//""+Local.Greenhouse

        public override string GetModuleDisplayName()
        {
            return "<size=1><color=#00000000>00</color></size>" + Local.Greenhouse;
        } // Attempt to display at top of tooltip//"Greenhouse"
