...
namespace Kerbalism.Modules
{
    public class KerbalismScansat : PartModule
    {
            ...
            else
            {
                ...
                if (IsScanning)
                {
                    ...
                    if (size > double.Epsilon)
                    {
                        ...
                        if (size > double.Epsilon)
                        {
                            ...
                            Message.Post(Lib.Color(Local.Scansat_Scannerhalted, Lib.Kolor.Red, true),
                                Local.Scansat_Scannerhalted_text.Format("<b>" + vessel.vesselName +
                                                                        "</b>")); //"Scanner halted""Scanner halted on <<1>>. No storage left on vessel."
                        }
                    }
                }
                else if (vd.scansat_id.Contains(part.flightID))
                {
                    if (vd.DrivesFreeSpace / vd.DrivesCapacity > 0.9) // restart when 90% of capacity is available
                    {
                        ...
                        if (vd.cfg_ec)
                            Message.Post(
                                Local.Scansat_sensorresumed.Format("<b>" + vessel.vesselName +
                                                                   "</b>")); //Lib.BuildString("SCANsat sensor resumed operations on <<1>>)
                    }
                }
            }
        }
        ...
        public static void BackgroundUpdate(Vessel vessel, ProtoPartSnapshot p, ProtoPartModuleSnapshot m,
            KerbalismScansat kerbalismScansat,
            Part part_prefab, VesselData vd, ResourceInfo ec, double elapsed_s)
        {
            List<ProtoPartModuleSnapshot> scanners =
                Cache.VesselObjectsCache<List<ProtoPartModuleSnapshot>>(vessel, "scansat_" + p.flightID);
            if (scanners == null)
            {
            ...
            if (!Features.Science)
            {
                if (is_scanning && ec.Amount < double.Epsilon)
                {
                    ...
                    if (vd.cfg_ec)
                        Message.Post(
                            Local.Scansat_sensordisabled.Format("<b>" + vessel.vesselName +
                                                                "</b>")); //Lib.BuildString("SCANsat sensor was disabled on <<1>>)
                }
            ...
            else
            {
                ...
                if (is_scanning)
                {
                    ...
                    SubjectData subject =
                        ScienceDB.GetSubjectData(expInfo, vd.VesselSituations.GetExperimentSituation(expInfo));
                    if (subject == null)
                        return;
