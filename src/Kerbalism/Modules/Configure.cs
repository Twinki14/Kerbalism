...
namespace Kerbalism.Modules
{
    ...
    public class Configure : PartModule, IPartCostModifier, IPartMassModifier, IModuleInfo, ISpecifics, IConfigurable
    {
        ...
        public override void OnStart(StartState state)
        {
            ...
            Events["ToggleWindow"].guiName =
                Lib.BuildString(Local.Module_Configure, " <b>", title, "</b>"); //"Configure"
        ...
        private void RemoveSymmetryOnVisibleResourceSwitch(string resName)
        {
            ...
            Message.Post(Local.RemoveSymmetry_msg.Format(part.partInfo.title,
                res.displayName)); //Lib.BuildString("Symmetry on <<1>>\nhas been removed because of switching the <<2>> capacity.")
        }
        ...
        [KSPEvent(guiActive = true, guiActiveUnfocused = true, guiActiveEditor = true, guiName = "_", active = false,
            groupName = "Configuration", groupDisplayName = "#KERBALISM_Group_Configuration")] //Configuration
            ...
            foreach (var pair in org)
            {
                ...
                foreach (string setup_title in setup_titles)
                {
                    specs.Add(Lib.BuildString("• <b>", setup_title, "</b>\n   at ",
                        Lib.Color(tech_title, Lib.Kolor.Science)));
                }
            }
        ...
        void Window_body(Panel p)
        {
            ...
            // set metadata
            p.Title(Lib.BuildString(Local.Module_Configure, " ", "<color=#cccccc>",
                Lib.Ellipsis(title, Styles.ScaleStringLength(40)), "</color>")); //Configure
        ...
        void Render_panel(Panel p, ConfigureSetup setup, int selected_i, int setup_i)
        {
            ...
            {
                p.AddSection(Lib.Ellipsis(setup.title, Styles.ScaleStringLength(70)), setup.desc,
                    () => Change_setup(-1, selected_i, ref setup_i), () => Change_setup(1, selected_i, ref setup_i));
            }
        ...
        {
            return Lib.BuildString("<size=1><color=#00000000>00</color></size>", Local.Module_Configurable, " ", title);
        } // attempt to display at the top//Configurable
    ...
    public class ConfigureSetup
    {
        ...
        public void Generate_details(Configure cfg)
        {
            ...
            if (visible_resources.Count > 0)
            {
                ...
                {
                    // add capacity info
                    details.Add(new Detail(Lib.GetResourceDisplayName(cr.name),
                        Lib.Parse.ToDouble(cr.maxAmount).ToString("F2")));
                }
            }
            ...
            if (mass > double.Epsilon || cost > double.Epsilon)
            {
                ...
                if (mass > double.Epsilon)
                    details.Add(new Detail(Local.Module_mass, Lib.HumanReadableMass(mass))); //"mass"
                if (cost > double.Epsilon)
                    details.Add(new Detail(Local.Module_cost, Lib.HumanReadableCost(cost))); //"cost"
            }
        }
