...
namespace Kerbalism.Modules
{
        ...
        IPartCostModifier
    {
        ...
        public string
            experiment_id =
                string.Empty; // if set, restricts write access to the experiment on the same part, with the given experiment_id.
        ...
        public double effectiveDataCapacity = -1.0; // effective drive capacity, in Mb. -1 = unlimited

        [KSPField(isPersistant = true)]
        public int effectiveSampleCapacity = -1; // effective drive capacity, in slots. -1 = unlimited

        [KSPField(isPersistant = false, guiName = "#KERBALISM_HardDrive_DataCapacity", guiActive = false,
             guiActiveEditor = false, groupName = "Science", groupDisplayName = "#KERBALISM_Group_Science"),
         UI_ChooseOption(scene = UI_Scene.Editor)]
        //Data Capacity--Science
        public string dataCapacityUI = "0";

        [KSPField(isPersistant = false, guiName = "#KERBALISM_HardDrive_SampleCapacity", guiActive = false,
             guiActiveEditor = false, groupName = "Science", groupDisplayName = "#KERBALISM_Group_Science"),
         UI_ChooseOption(scene = UI_Scene.Editor)]
        //Sample Capacity--Science
        public string sampleCapacityUI = "0";

        [KSPField(guiActive = true, guiName = "#KERBALISM_HardDrive_Capacity", guiActiveEditor = true,
            groupName = "Science", groupDisplayName = "#KERBALISM_Group_Science")]
        //Capacity--Science
            ...
            if (drive == null)
            {
                if (!Lib.IsFlight())
                {
                    drive = new Drive(title, effectiveDataCapacity, effectiveSampleCapacity,
                        !string.IsNullOrEmpty(experiment_id));
                }
                else
                {
                    ...
                    if (pd.Drive == null)
                    {
                        drive = new Drive(part.partInfo.title, effectiveDataCapacity, effectiveSampleCapacity,
                            !string.IsNullOrEmpty(experiment_id));
                        pd.Drive = drive;
                    }
                    else
        ...
        protected List<KeyValuePair<string, double>> GetDataCapacitySizes()
        {
            ...
            for (var i = 1; i <= maxDataCapacityFactor; i++)
                result.Add(new KeyValuePair<string, double>(Lib.HumanReadableDataSize(dataCapacity * i),
                    dataCapacity * i));
            return result;
        }
        ...
        protected List<KeyValuePair<string, int>> GetSampleCapacitySizes()
        {
            ...
            for (var i = 1; i <= maxSampleCapacityFactor; i++)
                result.Add(new KeyValuePair<string, int>(Lib.HumanReadableSampleSize(sampleCapacity * i),
                    sampleCapacity * i));
            return result;
        }
        ...
        public void Update()
        {
            ...
            if (part.IsPAWVisible() && Lib.IsFlight())
            {
                ...
                Events["ToggleUI"].guiName = Lib.StatusToggle(Local.HardDrive_Data,
                    drive.Empty() ? Local.HardDrive_Dataempty : drive.Size()); //"Data""empty"
        ...
        [KSPEvent(guiActive = true, guiName = "_", active = true, groupName = "Science",
        ...
        [KSPEvent(guiName = "#KERBALISM_HardDrive_TransferData", active = false, groupName = "Science",
            groupDisplayName = "#KERBALISM_Group_Science")] //Science
        ...
        [KSPEvent(guiActive = false, guiActiveUnfocused = true, guiActiveUncommand = true,
            guiName = "#KERBALISM_HardDrive_TakeData", active = true, groupName = "Science",
            groupDisplayName = "#KERBALISM_Group_Science")] //Science
        public void TakeData()
        {
            ...
            if (!Drive.Transfer(drive, v, PreferencesScience.Instance.sampleTransfer || Lib.CrewCount(v) > 0))
            {
                Message.Post
                (
                    Lib.Color(Lib.BuildString(Local.HardDrive_WARNING_title), Lib.Kolor.Red,
                        true), //"WARNING: not evering copied"
        ...
        [KSPEvent(guiActive = false, guiActiveUnfocused = true, guiActiveUncommand = true,
            guiName = "#KERBALISM_HardDrive_TransferData", active = true, groupName = "Science",
            groupDisplayName = "#KERBALISM_Group_Science")] //Science
        ...
        public void ReturnData(ScienceData data)
        {
            ...
            if (data.baseTransmitValue > Science.Science.maxXmitDataScalarForSample ||
                data.transmitBonus > Science.Science.maxXmitDataScalarForSample)
            {
        ...
        public Specifics Specs()
        {
            ...
            specs.Add(Local.HardDrive_info1,
                dataCapacity >= 0
                    ? Lib.HumanReadableDataSize(dataCapacity)
                    : Local.HardDrive_Capacityunlimited); //"File capacity""unlimited"
            specs.Add(Local.HardDrive_info2,
                sampleCapacity >= 0
                    ? Lib.HumanReadableSampleSize(sampleCapacity)
                    : Local.HardDrive_Capacityunlimited); //"Sample capacity""unlimited"
            return specs;
        }
        ...
        public float GetModuleMass(float defaultMass, ModifierStagingSituation sit)
        {
            ...
            {
                Lib.Log("Drive mass is NaN: esc " + effectiveSampleCapacity + " scm " + sampleCapacityMass + " dedcm " +
                        effectiveDataCapacity + " dcm " + dataCapacityMass + " tsm " + totalSampleMass);
                return 0;
            }
