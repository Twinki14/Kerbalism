...
namespace Kerbalism
{
    public static class Lib
    {
        ...
        private static void Log(MethodBase method, string message, LogLevel level)
        {
            ...
            {
                default:
                    UnityEngine.Debug.Log(string.Format("[Kerbalism] {0}.{1} {2}", method.ReflectedType.Name,
                        method.Name, message));
                    return;
                case LogLevel.Warning:
                    UnityEngine.Debug.LogWarning(string.Format("[Kerbalism] {0}.{1} {2}", method.ReflectedType.Name,
                        method.Name, message));
                    return;
                case LogLevel.Error:
                    UnityEngine.Debug.LogError(string.Format("[Kerbalism] {0}.{1} {2}", method.ReflectedType.Name,
                        method.Name, message));
                    return;
            }
        }
        ...
        public static Version KerbalismVersion
        {
            get
            {
                if (kerbalismVersion == null)
                    kerbalismVersion =
                        new Version(Assembly.GetAssembly(typeof(Kerbalism.System.Kerbalism)).GetName().Version.Major,
                            Assembly.GetAssembly(typeof(Kerbalism.System.Kerbalism)).GetName().Version.Minor);
        ...
        {
            get { return Versioning.version_major.ToString() + Versioning.version_minor.ToString(); }
        }
        ...
        public static int KerbalismDevBuild
        {
            get
            {
                ...
                {
                    AssemblyLoader.LoadedAssembly bootstrap =
        ...
        public static double PrincipiaCorrectInclination(Orbit o)
        {
            if (HasPrincipia && o.referenceBody != (FlightGlobals.currentMainBody ?? Planetarium.fetch.Home))
            {
                ...
                else
                {
                    ...
                    return Math.Atan2((orbitZ - polarAxis).magnitude, (orbitZ + polarAxis).magnitude) *
                           (2d * (180.0 / Math.PI));
                }
            }
            else
        ...
        public static string KerbalismRootPath =>
            Path.Combine(Path.Combine(Path.GetFullPath(KSPUtil.ApplicationRootPath), "GameData"), "Kerbalism");
        ...
        public static string ExpandMsg(string txt, Vessel v = null, ProtoCrewMember c = null, uint variant = 0)
        {
            ...
            return txt
                ...
                .Replace("$HIS_HER",
                    c != null && c.gender == ProtoCrewMember.Gender.Male
                        ? Local.Kerbal_his
                        : Local.Kerbal_her); //"his""her"
        }
        ...
        public static string KolorToHex(Kolor color)
        {
            switch (color)
            {
                ...
                case Kolor.Green:
                    return
                        "#88FF00"; // green whith slightly less red than the ksp ui default (CCFF00), for better contrast with yellow
        ...
        {
            return !bold
                ? BuildString("<color=", KolorToHex(color), ">", s, "</color>")
                : BuildString("<color=", KolorToHex(color), "><b>", s, "</b></color>");
        }
        ...
        public static string Color(bool condition, string s, Kolor colorIfTrue, Kolor colorIfFalse = Kolor.None,
            bool bold = false)
        {
            return condition ? Color(s, colorIfTrue, bold) :
                colorIfFalse == Kolor.None ? bold ? Bold(s) : s : Color(s, colorIfFalse, bold);
        }
        ...
        public static string Color(bool condition, string sIfTrue, Kolor colorIfTrue, string sIfFalse,
            Kolor colorIfFalse = Kolor.None, bool bold = false)
        {
            return condition ? Color(sIfTrue, colorIfTrue, bold) :
                colorIfFalse == Kolor.None ? bold ? Bold(sIfFalse) : sIfFalse : Color(sIfFalse, colorIfFalse, bold);
        }
        ...
        {
            return Regex.Replace(s, "[A-Z]", " $0").TrimStart();
        }
...
#pragma warning disable CS0649
            ...
            [Persistent] private string rateUnit;
            public string RateUnit => rateUnit;
            [Persistent] private bool useRatePostfix = true;
            public bool UseRatePostfix => useRatePostfix;
            [Persistent] private string amountUnit;
            public string AmountUnit => amountUnit;
            [Persistent] private double multiplierToUnit = 1d;
            public double MultiplierToUnit => multiplierToUnit;
            [Persistent] private bool useHuman = false;
...
#pragma warning restore CS0649
        ...
        private static readonly Dictionary<int, ResourceUnitInfo> resourceUnitInfos =
            new Dictionary<int, ResourceUnitInfo>();
        ...
        public static string HumanOrSIRate(double rate, string unit, int sigFigs = 3, string precision = "F3",
        ...
        public static string HumanOrSIRate(double rate, int resID, int sigFigs = 3, string precision = "F3",
            bool longPrefix = false)
        {
        ...
        public static string HumanOrSIAmount(double amount, int resID, int sigFigs = 3, string append = "",
            bool longPrefix = false)
        {
            if (Settings.UseSIUnits && GetResourceUnitInfo(resID) is ResourceUnitInfo rui)
            {
        ...
        public const string InlineSpriteReputation =
            "<sprite=\"CurrencySpriteAsset\" name=\"Reputation\" color=#E0D503>";
        ...
        public static string HumanReadableAmount(double value, string append = "")
        {
            return (Math.Abs(value) <= double.Epsilon
                ? Local.Generic_NONE
                : BuildString(value.ToString("F2"), append)); //"none"
        }
        ...
        private static Bounds GetTransformRootAndChildrensBounds(Transform transform)
        {
            ...
            UnityEngine.Renderer[] renderers = transform.GetComponentsInChildren<UnityEngine.Renderer>(false);
        ...
        public class PartVolumeAndSurfaceInfo
        {
            ...
            public void GetUsingBestMethod(out double volume, out double surface,
                bool substractAttachNodesSurface = true)
            {
                GetUsingMethod(bestMethod, out volume, out surface, substractAttachNodesSurface);
            }

            public void GetUsingMethod(VolumeAndSurfaceMethod method, out double volume, out double surface,
                bool substractAttachNodesSurface = true)
            {
                switch (method)
                {
                    case VolumeAndSurfaceMethod.Bounds:
                        volume = boundsVolume;
                        surface = substractAttachNodesSurface
                            ? SubstractNodesSurface(boundsSurface, attachNodesSurface)
                            : boundsSurface;
                        return;
                    case VolumeAndSurfaceMethod.Collider:
                        volume = colliderVolume;
                        surface = substractAttachNodesSurface
                            ? SubstractNodesSurface(colliderSurface, attachNodesSurface)
                            : colliderSurface;
                        return;
                    case VolumeAndSurfaceMethod.Mesh:
                        volume = meshVolume;
                        surface = substractAttachNodesSurface
                            ? SubstractNodesSurface(meshSurface, attachNodesSurface)
                            : meshSurface;
                        return;
                    default:
                        volume = 0.0;
        ...
        private struct MeshInfo : IEquatable<MeshInfo>
        {
            ...
            {
                return
                    $"\"{name}\" : VOLUME={volume.ToString("0.00m3")} - SURFACE={surface.ToString("0.00m2")} - BOUNDS VOLUME={boundsVolume.ToString("0.00m3")}";
            }
            ...
            Transform rootTransform = null)
        {
            ...
            int usedMeshCount =
                GetMeshesTotalVolumeAndSurface(meshInfos, out results.meshVolume, out results.meshSurface, logAll);
            ...
            int usedCollidersCount = GetMeshesTotalVolumeAndSurface(colliderMeshInfos, out results.colliderVolume,
                out results.colliderSurface, logAll);
            ...
            bool colliderIsValid = results.colliderVolume < results.boundsVolume * validityFactor &&
                                   results.colliderVolume > 0.25;
            ...
            else
            {
                ...
                double volumeDifference = Math.Abs(results.colliderVolume - results.meshVolume) /
                                          Math.Max(results.colliderVolume, results.meshVolume);
                ...
                if (volumeDifference < 0.2 && (usedCollidersCount != usedMeshCount))
                    results.bestMethod = usedCollidersCount < usedMeshCount
                        ? VolumeAndSurfaceMethod.Collider
                        : VolumeAndSurfaceMethod.Mesh;
                ...
                // in other cases, the method that return the largest volume is usually right
                    ...
                    results.bestMethod = results.colliderVolume > results.meshVolume
                        ? VolumeAndSurfaceMethod.Collider
                        : VolumeAndSurfaceMethod.Mesh;
            }
            ...
            foreach (AttachNode attachNode in part.attachNodes)
            {
                ...
                {
                    case 0:
                        results.attachNodesSurface += 0.3068;
                        break; // 0.625 m disc
                    case 1:
                        results.attachNodesSurface += 1.2272;
                        break; // 1.25 m disc
                    case 2:
                        results.attachNodesSurface += 4.9090;
                        break; // 2.5 m disc
                    case 3:
                        results.attachNodesSurface += 11.045;
                        break; // 3.75 m disc
                    case 4:
                        results.attachNodesSurface += 19.635;
                        break; // 5 m disc
                }
            }
            ...
            if (logAll)
            {
                ...
                Log(
                    $"Bounds method :   Volume:{results.boundsVolume.ToString("0.00m3")} - Surface:{results.boundsSurface.ToString("0.00m2")} - Max valid volume:{(results.boundsVolume * validityFactor).ToString("0.00m3")}");
                Log(
                    $"Collider method : Volume:{results.colliderVolume.ToString("0.00m3")} - Surface:{results.colliderSurface.ToString("0.00m2")} - Raw volume:{rawColliderVolume.ToString("0.00m3")} - Raw surface:{rawColliderSurface.ToString("0.00m2")} - Meshes: {usedCollidersCount}/{colliderCount} (valid/raw)");
                Log(
                    $"Mesh method :     Volume:{results.meshVolume.ToString("0.00m3")} - Surface:{results.meshSurface.ToString("0.00m2")} - Raw volume:{rawMeshVolume.ToString("0.00m3")} - Raw surface:{rawMeshSurface.ToString("0.00m2")} - Meshes: {usedMeshCount}/{meshCount} (valid/raw)");
                Log($"Attach nodes surface : {results.attachNodesSurface.ToString("0.00m2")}");
                Log(
                    $"Returned result : Volume:{volume.ToString("0.00m3")} - Surface:{surface.ToString("0.00m2")} - Method used : {results.bestMethod.ToString()}");
            }

            return results;
        }

        private static int GetMeshesTotalVolumeAndSurface(List<MeshInfo> meshInfos, out double volume,
            out double surface, bool logAll = false)
        {
            volume = 0.0;
            surface = 0.0;
            ...
            for (int i = 0; i < meshInfos.Count; i++)
            {
                ...
                if (meshInfo.volume < minMeshVolume)
                {
                    if (logAll)
                        Lib.Log(
                            $"Found {meshInfo.ToString()} - INTERSECTED VOLUME={intersectedVolume.ToString("0.00m3")} - Mesh rejected : too small");
                    continue;
                }
                ...
                if (i > 0 && intersectedVolume / meshInfo.boundsVolume > 0.75)
                {
                    if (logAll)
                        Lib.Log(
                            $"Found {meshInfo.ToString()} - INTERSECTED VOLUME={intersectedVolume.ToString("0.00m3")} - Mesh rejected : it is inside another mesh");
                    continue;
                }

                if (logAll)
                    Lib.Log(
                        $"Found {meshInfo.ToString()} - INTERSECTED VOLUME={intersectedVolume.ToString("0.00m3")} - Mesh accepted");
                usedMeshesCount++;
        ...
        private static List<MeshInfo> GetPartMeshesVolumeAndSurface(Transform partRootTransform,
            bool ignoreSkinnedMeshes)
        {
            ...
            if (!ignoreSkinnedMeshes)
            {
                SkinnedMeshRenderer[] skinnedMeshRenderers =
                    partRootTransform.GetComponentsInChildren<SkinnedMeshRenderer>(false);
        ...
        public static bool HasPart(Vessel v, string part_name)
        {
            ...
            foreach (string name in Tokenize(part_name, ','))
            {
                if (v.loaded)
                    ...
                    ret = v.protoVessel.protoPartSnapshots.Find(k =>
                        k.partName.StartsWith(part_name, StringComparison.Ordinal)) != null;
                if (ret) break;
            }
        ...
        /// </summary>
        public static PartModule ModulePrefab(List<PartModule> module_prefabs, string module_name,
            Dictionary<string, Module_prefab_data> PD)
        {
        ...
        public static void SetResourceCapacity(Part p, string res_name, double capacity)
        {
            ...
            {
                Lib.Log(
                    Lib.BuildString("error while setting capacity for ", res_name, ": the resource is not in the part"),
                    Lib.LogLevel.Error);
                return;
            }
...
#pragma warning restore CS0618
        ...
        public static class Proto
        {
            ...
            public static double GetDouble(ProtoPartModuleSnapshot m, string name, double def_value = 0.0)
            {
                ...
                double v;
                string s = m.moduleValues.GetValue(name);
                return s != null && double.TryParse(s, out v) && !double.IsNaN(v) && !double.IsInfinity(v)
                    ? v
                    : def_value;
            }
