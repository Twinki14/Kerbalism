...
namespace Kerbalism
{
    public class ScienceArchiveWindow
    {
        ...
        private void Init()
        {
            ...
            researchFilter = new KsmGuiToggle(experimentColumn, Local.SCIENCEARCHIVE_filter1, true,
                OnToggleResearchedFilter); //"filter by researched"
            if (Kerbalism.System.Kerbalism.SerenityEnabled)
                ROCFilter = new KsmGuiToggle(experimentColumn, Local.SCIENCEARCHIVE_filter2, true,
                    OnToggleROCFilter); //"filter ROCs"
            vesselFilter =
                new KsmGuiToggle(experimentColumn, Local.SCIENCEARCHIVE_filter3, false,
                    OnToggleVesselFilter); //"filter by current vessel"

            KsmGuiVerticalScrollView experimentsScrollView =
                new KsmGuiVerticalScrollView(experimentColumn, 0, 0, 0, 0, 0);
        ...
        private static void OnToggleExperiment(ExpInfoAndSubjects expInfoAndSubjects)
        {
            ...
            expInfoAndSubjects.experimentSubjectList.KnownSubjectsToggle.SetOnState(
                currentExperiment.experimentSubjectList.KnownSubjectsToggle.IsOn, true);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private static void UpdateResearchedFilter()
        {
            ...
            foreach (AvailablePart availablePart in PartLoader.LoadedPartsList)
            {
                ...
                if (!string.IsNullOrEmpty(availablePart.TechRequired) &&
                    !ResearchAndDevelopment.PartModelPurchased(availablePart))
                    continue;
