...
namespace Kerbalism
{
    public static class Sim
    {
        ...
        private static double FracEclipseCircular(double betaAngle, double sma, double R)
        {
            ...
            return (1.0 / Math.PI) *
                   Math.Acos(Math.Sqrt(avgHeight * avgHeight + 2.0 * R * avgHeight) / (sma * Math.Cos(betaAngle)));
        }
        ...
        /// <returns></returns>
        private static double FracEclipseElliptical(Vessel v, double betaAngle, double a, double R, double e,
            Vector3d sunVec)
        {
            var obt = v.orbit;
        ...
        /// <returns></returns>
        internal static bool IsSunVisibleAtTime(Vessel vessel, Vector3d vesselPos, CelestialBody sun,
            List<CelestialBody> occluders, bool isSurf)
        {
        ...
        public static bool RaytracePhysic(Vessel vessel, Vector3d vesselPos, Vector3d end, double endNegOffset = 0.0)
        {
            ...
            if (!vessel.loaded)
                vesselPos += vessel.mainBody.position -
                             vessel.mainBody.getTruePositionAtUT(Planetarium.GetUniversalTime() +
        ...
        /// <returns></returns>
        public static bool IsBodyVisible(Vessel vessel, Vector3d vesselPos, CelestialBody body,
            List<CelestialBody> occludingBodies, out Vector3d bodyDir, out double bodyDist)
        {
            // generate ray parameters
            ...
            if (Lib.Landed(vessel) && vessel.mainBody.Radius < 100000.0 &&
                (vessel.latitude < -45.0 || vessel.latitude > 45.0))
        ...
        public class BodyCache
        {
            ...
            public Vector3d GetBodyPosition(int idx)
            {
        ...
        private static Vector3d FastGetRelativePositionAtUT(Orbit orbit, double UT, double semiLatusRectum)
        {
            ...
            Vector3d pos = semiLatusRectum / (1.0 + orbit.eccentricity * cos) *
                           (orbit.OrbitFrame.X * cos + orbit.OrbitFrame.Y * sin);
        ...
        /// <returns></returns>
        public static double SolarFluxAtBody(CelestialBody body, bool worstCase, out CelestialBody mainSun,
            out Vector3d mainSunDirection, out double mainSunDistance)
        {
        ...
        public static double Temperature(Vessel v, Vector3d position, double solar_flux, out double albedo_flux,
            out double body_flux, out double total_flux)
        {
            // get vessel body
        ...
        public static double AtmosphereFactorAnalytic(CelestialBody body, Vector3d position, Vector3d sun_dir)
        {
            ...
            if (static_pressure > 0.0)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    body.GetSolarAtmosphericEffects(Vector3d.Dot(radialOut, sunDirs[i]), density, out _,
                        out double stockFluxFactor);
        ...
        {
            var body = v.mainBody;
            return body.atmosphere && v.altitude > body.atmosphereDepth * 5.0 &&
                   v.altitude <= body.atmosphereDepth * 25.0;
        }
        ...
        public static double DataRateDampingExponent
        {
            get
            {
                ...
                Lib.Log(
                    $"Calculated DataRateDampingExponent: {dampingExponent.ToString("F4")} (max. DSN range: {maxDsnRange.ToString("F0")}, strength at 2 AU: {strengthAt2AU.ToString("F3")})");
