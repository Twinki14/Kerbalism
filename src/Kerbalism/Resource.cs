...
namespace Kerbalism
{
    public class ResourceBroker
    {
        ...
        public static ResourceBroker Greenhouse =
            GetOrCreate("Greenhouse", BrokerCategory.VesselSystem, Local.Brokers_Greenhouse);
        ...
        public static ResourceBroker Command =
            GetOrCreate("Command", BrokerCategory.VesselSystem, Local.Brokers_Command);

        public static ResourceBroker GravityRing =
            GetOrCreate("GravityRing", BrokerCategory.RTG, Local.Brokers_GravityRing);

        public static ResourceBroker Scanner =
            GetOrCreate("Scanner", BrokerCategory.VesselSystem, Local.Brokers_Scanner);

        public static ResourceBroker Laboratory =
            GetOrCreate("Laboratory", BrokerCategory.Science, Local.Brokers_Laboratory);

        public static ResourceBroker
            CommsIdle = GetOrCreate("CommsIdle", BrokerCategory.Comms, Local.Brokers_CommsIdle);
    ...
    public sealed class VesselResources
    {
        ...
        private ResourceInfo GetResInfo(string resName)
        {
            resources.TryGetValue(resName.GetHashCode(), out var ri);
            return ri;
        }
            ...
            if (v.loaded)
            {
                foreach (Part p in v.Parts)
                {
                    ...
                    foreach (PartResource r in p.Resources.dict.Values)
                    {
                        ...
                        {
                            int pri = r.info.resourceFlowMode == ResourceFlowMode.ALL_VESSEL_BALANCE ||
                                      r.info.resourceFlowMode == ResourceFlowMode.ALL_VESSEL
                                ? 1
                                : partPri;
            ...
            else
            {
                foreach (ProtoPartSnapshot p in v.protoVessel.protoPartSnapshots)
                {
                    int partPri = Settings.UseResourcePriority
                        ? ((p.partInfo.partPrefab.resourcePriorityUseParentInverseStage
                            ? p.parent.inverseStageIndex
                            : p.inverseStageIndex) * 10 + p.resourcePriorityOffset)
                        : 1;
                    foreach (ProtoPartResourceSnapshot r in p.resources)
                    {
                        if (r.flowState && resources.TryGetValue(r.definition.id, out var resInfo))
                        {
                            int pri = r.definition.resourceFlowMode == ResourceFlowMode.ALL_VESSEL_BALANCE ||
                                      r.definition.resourceFlowMode == ResourceFlowMode.ALL_VESSEL
                                ? 1
                                : partPri;
                            resInfo.AddToSyncSet(r, pri);
                        }
    ...
    public sealed class ResourceInfo
    {
        ...
        public class WrapPR : Wrap
        {
            ...
            {
                get => res.amount;
                set => res.amount = value;
            }

            public override double maxAmount
            {
                get => res.maxAmount;
                set => res.maxAmount = value;
            }

            public void Link(PartResource r)
            {
                res = r;
            }
        ...
        public class WrapPPRS : Wrap
        {
            ...
            public void Link(ProtoPartResourceSnapshot r)
            {
                res = r;
            }
        ...
        public void Sync(Vessel v, VesselData vd, double elapsed_s)
        {
            ...
            if (!v.loaded || !Kerbalism.System.Kerbalism.WarpBlending)
            ...
            vd.Supply(ResourceName).UpdateResourceBrokers(brokersResourceAmounts, intervalRuleBrokersRates,
                unsupportedBrokersRate, elapsed_s);
            ...
            if (Settings.EnforceCoherency && v.loaded && TimeWarp.CurrentRate > 1000.0 &&
                unsupportedBrokersRate > 0.0 && !flowStateChanged)
            {
                Message.Post
                (
                    Severity.warning,
    ...
    public sealed class ResourceRecipe
    {
        public struct Entry
        {
            ...
            public string
                combined; // if entry is the primary to be combined, then the secondary resource is named here. secondary entry has its combined set to "" not null
        ...
        private bool ExecuteRecipeStep(Vessel v, VesselResources resources)
        {
                ...
                for (int i = 0; i < inputs.Count; ++i)
                {
                    ...
                    if (e.combined != null)
                    {
                        ...
                        if (e.combined != "")
                        {
                            ...
                            double pri_worst = Lib.Clamp((res.Amount + res.Deferred) * e.inv_quantity, 0.0,
                                worst_input);
                            if (pri_worst > 0.0)
                            {
                            ...
                            {
                                worst_input = Lib.Clamp((sec.Amount + sec.Deferred) * sec_e.inv_quantity, 0.0,
                                    worst_input);
                            }
                        }
                    }
            ...
            if (inputs.Count > 0)
            {
                for (int i = 0; i < outputs.Count; ++i)
                {
                    ...
                    {
                        ResourceInfo res = resources.GetResource(v, e.name);
                        worst_output = Lib.Clamp((res.Capacity - (res.Amount + res.Deferred)) * e.inv_quantity, 0.0,
                            worst_output);
                    }
                }
            }
        ...
        public static void Equalizer(Vessel v)
        {
            ...
            for (int i = 0; i < resourceName.Length; i++)
            {
                ...
                if (mannedisPriority[i])
                    amount[i] = maxAmount[i] * equalize_speed * Kerbalism.System.Kerbalism.elapsed_s;
                else
                    amount[i] = (maxE[i] + maxD[i]) * equalize_speed * Kerbalism.System.Kerbalism.elapsed_s;
            }
                    ...
                    if (partHabitat.state != Habitat.State.disabled)
                    {
                        for (int i = 0; i < resourceName.Length; i++)
                        {
                            if (partHabitat.part.Resources.Contains(resourceName[i]))
                            {
                                ...
                                if ((Math.Abs(res_level[i] - (t.amount / t.maxAmount)) > precision &&
                                     !Lib.IsCrewed(partHabitat.part))
                                         ...
                                         || mannedisPriority[i]) && t.amount > double.Epsilon))
                                {
                                    ...
                                    {
                                        perctRest = (((totalAmount[i] - maxP[i]) * perctToMaxType) - t.amount) /
                                                    totalE[i];
                                    }
                                    ...
                                    if ((partHabitat.state == Habitat.State.depressurizing ||
                                         totalAmount[i] - maxP[i] <= 0) && !Lib.IsCrewed(partHabitat.part))
                                    {
                                        flowController = 0 - perctToType;
                                    }
                                    else if (mannedisPriority[i] && !Lib.IsCrewed(partHabitat.part))
                                    {
                                        flowController = Math.Min(perctToMaxType - perctToAll,
                                            (t.maxAmount - t.amount) / totalAmount[i]);
                                    }
                                    else
                                    {
                                    ...
                                    amountAffected = Math.Sign(amountAffected) >= 0
                                        ? Math.Max(Math.Sign(amountAffected) * precision, amountAffected)
                                        : Math.Min(Math.Sign(amountAffected) * precision, amountAffected);

                                    double va = amountAffected < 0.0
                                        ? Math.Abs(amountAffected) >
                                          t.amount // If negative, habitat can't send more than it has
                                            ? t.amount * (-1)
                                            : amountAffected
                                        : Math.Min(amountAffected,
                                            t.maxAmount - t.amount); // if positive, habitat can't receive more than max
