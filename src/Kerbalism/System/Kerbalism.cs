        ...
        public static void ManageRescueMission(Vessel v)
        {
            ...
            foreach (ProtoCrewMember c in Lib.CrewList(v))
            {
                ...
                if (kd.rescue)
                {
                    ...
                    // show a message
                    Message.Post(Lib.BuildString(Local.Rescuemission_msg1, " <b>", c.name, "</b>"),
                        Lib.BuildString(
                            (c.gender == ProtoCrewMember.Gender.Male ? Local.Kerbal_Male : Local.Kerbal_Female),
                            Local.Rescuemission_msg2)); //We found xx  "He"/"She"'s still alive!"
                }
            }
            ...
            if (detected)
            {
                ...
                global::Kerbalism.ResourceCache.Produce(v, monoprop_name, monoprop_amount,
                    global::Kerbalism.ResourceBroker.Generic);

                // give the vessel some supplies
                Profile.Profile.SetupRescue(v);
            }
        }
