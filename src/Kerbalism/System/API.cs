...
namespace Kerbalism.System
{
    ...
    public static class API
    {
        ...
        {
            if (!Features.Radiation) return false;
            RadiationBody rb = global::Kerbalism.Radiation.Info(body);
        ...
        {
            if (!Features.Radiation) return 0.0;
            var info = global::Kerbalism.Radiation.Info(body);
        ...
        public class RadiationFieldChanged
        {
            ...
            public void Add(Action<Vessel, bool, bool, bool> receiver)
            {
                if (!receivers.Contains(receiver)) receivers.Add(receiver);
            }

            public void Remove(Action<Vessel, bool, bool, bool> receiver)
            {
        ...
        {
            global::Kerbalism.ResourceCache.Consume(v, resource_name, quantity,
                global::Kerbalism.ResourceBroker.GetOrCreate(title));
        }

        public static void ProduceResource(Vessel v, string resource_name, double quantity, string title)
        {
            global::Kerbalism.ResourceCache.Produce(v, resource_name, quantity,
                global::Kerbalism.ResourceBroker.GetOrCreate(title));
        }

        public static void ProcessResources(Vessel v, List<KeyValuePair<string, double>> resources, string title)
        {
            global::Kerbalism.ResourceBroker broker = global::Kerbalism.ResourceBroker.GetOrCreate(title);
            foreach (var p in resources)
            {
                if (p.Value < 0)
                    global::Kerbalism.ResourceCache.Consume(v, p.Key, -p.Value, broker);
                else
                    global::Kerbalism.ResourceCache.Produce(v, p.Key, p.Value, broker);
            }
        }

        public static double ResourceAmount(Vessel v, string resource_name)
        {
            return global::Kerbalism.ResourceCache.GetResource(v, resource_name).Amount;
        }
        ...
        public static List<double> ResourceAmounts(Vessel v, List<string> resource_names)
        {
            ...
            foreach (var name in resource_names)
                result.Add(global::Kerbalism.ResourceCache.GetResource(v, name).Amount);
            return result;
        }

        public static double ResourceCapacity(Vessel v, string resource_name)
        {
            return global::Kerbalism.ResourceCache.GetResource(v, resource_name).Capacity;
        }
        ...
        public static List<double> ResourceCapacities(Vessel v, List<string> resource_names)
        {
            List<double> result = new List<double>(resource_names.Count);
            foreach (var name in resource_names)
                result.Add(global::Kerbalism.ResourceCache.GetResource(v, name).Capacity);
            return result;
        }

        public static double ResourceLevel(Vessel v, string resource_name)
        {
            return global::Kerbalism.ResourceCache.GetResource(v, resource_name).Level;
        }
        ...
        public static List<double> ResourceLevels(Vessel v, List<string> resource_names)
        {
            List<double> result = new List<double>(resource_names.Count);
            foreach (var name in resource_names)
                result.Add(global::Kerbalism.ResourceCache.GetResource(v, name).Level);
            return result;
        }
        ...
        */
        ...
        public class ExperimentStateChanged
        {
            ...
            public void Add(Action<Vessel, string, bool> receiver)
            {
                if (!receivers.Contains(receiver)) receivers.Add(receiver);
            }

            public void Remove(Action<Vessel, string, bool> receiver)
            {
                if (receivers.Contains(receiver)) receivers.Remove(receiver);
            }

            public void Notify(Vessel vessel, string experiment_id, Experiment.ExpStatus oldStatus,
                Experiment.ExpStatus newStatus)
            {
                ...
                foreach (Action<Vessel, string, bool> receiver in receivers)
                {
                    ...
                    {
                        Lib.Log(
                            "ExperimentStateChanged: Exception in event receiver " + e.Message + "\n" + e.ToString());
                    }
                }
            }
        }
