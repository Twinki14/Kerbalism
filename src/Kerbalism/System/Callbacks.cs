...
namespace Kerbalism.System
{
    ...
    public sealed class Callbacks
    {
        ...
        public Callbacks()
        {
            ...
            // add editor events
            GameEvents.onEditorShipModified.Add((sc) => global::Kerbalism.Planner.Planner.EditorShipModifiedEvent(sc));
        }
        ...
        void ToEVA(GameEvents.FromToAction<Part, Part> data)
        {
            ...
            VesselResources resources = global::Kerbalism.ResourceCache.Get(data.from.vessel);

            // setup supply resources capacity in the eva kerbal
            Profile.Profile.SetupEva(data.to);
            ...
            for (int i = 0; i < data.to.Resources.Count; ++i)
            {
                ...
                double quantity = Math.Min(resources.GetResource(data.from.vessel, res.resourceName).Amount / tot_crew,
                    res.maxAmount);
        ...
        IEnumerator PostEVATweaks(Part vesselHatch, Part kerbalPart, string evaPropName)
        {
            ...
            if (kerbalEVA.ModuleInventoryPartReference != null)
            {
                foreach (StoredPart storedPart in kerbalEVA.ModuleInventoryPartReference.storedParts.Values)
                {
                    ...
                    ProtoPartResourceSnapshot prop =
                        storedPart.snapshot.resources.Find(p => p.resourceName == evaPropName);
                    if (prop != null)
                    {
            ...
            if (propContainers.Count > 0)
            {
                ...
                foreach (ProtoPartResourceSnapshot propContainer in propContainers)
                {
                    ...
                    {
                        double vesselPropTransferred = vesselHatch.RequestResource(evaPropName,
                            propContainer.maxAmount - propContainer.amount);
                        if (vesselPropTransferred > 0.0)
                        {
                            transferred = true;
                            propContainer.amount = Math.Min(propContainer.amount + vesselPropTransferred,
                                propContainer.maxAmount);
                        }
                    }
            ...
            if (hasJetPack && evaPropQuantity <= 0.05 && !Lib.Landed(vesselHatch.vessel))
            {
                Message.Post(Severity.danger,
                    Local.CallBackMsg_EvaNoMP.Format("<b>" + evaPropName + "</b>"),
                    Local.CallBackMsg_EvaNoMP2); //Lib.BuildString("There isn't any <<1>> in the EVA JetPack")"Don't let the ladder go!"
            }
        }
        ...
        void FromEVA(GameEvents.FromToAction<Part, Part> data)
        {
            ...
            if (evaPropName != "EVA Propellant")
            {
                ...
                if (kerbalEVA.ModuleInventoryPartReference != null)
                {
                    foreach (StoredPart storedPart in kerbalEVA.ModuleInventoryPartReference.storedParts.Values)
                    {
                        ProtoPartResourceSnapshot propContainer =
                            storedPart.snapshot.resources.Find(p => p.resourceName == evaPropName);
                ...
                {
                    // get vessel resources handler
                    ResourceInfo evaPropOnVessel =
                        global::Kerbalism.ResourceCache.GetResource(data.to.vessel, evaPropName);
                    ...
                    foreach (ProtoPartResourceSnapshot propContainer in propContainers)
                    {
                        ...
                        evaPropOnVessel.Produce(stored, global::Kerbalism.ResourceBroker.Generic);
                    ...
                    data.to.protoModuleCrew[data.to.protoModuleCrew.Count - 1]
                        .SaveInventory(kerbalEVA.ModuleInventoryPartReference);
                }
            }
#endif
        ...
        void VesselRecovered(ProtoVessel pv, bool b)
        {
            ...
            // purge the caches
            global::Kerbalism.ResourceCache.Purge(pv);
        ...
        void VesselDestroyed(Vessel v)
        {
            ...
            // purge the caches
            global::Kerbalism.ResourceCache.Purge(v); // works with loaded and unloaded vessels
        ...
        {
            if (PartLoader.LoadedPartsList.Find(k => k.tags.IndexOf("_kerbalism", StringComparison.Ordinal) >= 0) !=
                null)
            {
                RUI.Icons.Selectable.Icon icon = new RUI.Icons.Selectable.Icon("Kerbalism", Textures.category_normal,
                    Textures.category_selected);
                PartCategorizer.Category category = PartCategorizer.Instance.filters.Find(k =>
                    string.Equals(k.button.categoryName, "filter by function", StringComparison.OrdinalIgnoreCase));
                PartCategorizer.AddCustomSubcategoryFilter(category, "Kerbalism", "Kerbalism", icon,
